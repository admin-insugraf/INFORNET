...
Public Class frm_AF_MA_Marca
    Dim bol_nuevo As Boolean = False
    Private Sub frm_AF_MA_Marca_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call Cargar_Datos()
        Inicializa_Tool(ToolS_Mantenimiento)
    End Sub

    Private Sub Cargar_Datos()
        Dim cateBL As New ClsOperaciones.AF_MA_MARCA
        dgv_lista.DataSource = cateBL.get_marcas
        cateBL = Nothing
    End Sub
    ...
    Private Sub Tool_Grabar_Click(sender As Object, e As EventArgs) Handles Tool_Grabar.Click
        ...
        Dim catBE As New ClsEntidades.AF_MA_MARCA
        Dim catBL As New ClsOperaciones.AF_MA_MARCA

        With catBE
            .MA_ID = txt_cod.Text.Trim
            .MA_DESCRIPCION = txt_des.Text.Trim
            .MA_ESTADO = IIf(chk_estado.Checked, 1, 0)
        End With
    ...
    Private Sub Tool_Editar_Click(sender As Object, e As EventArgs) Handles Tool_Editar.Click
        ...
        Cambiar_Estado_Tool(ToolS_Mantenimiento)
        txt_cod.Text = dgv_lista.CurrentRow.Cells("MA_ID").Value
        txt_des.Text = dgv_lista.CurrentRow.Cells("MA_DESCRIPCION").Value
        chk_estado.Checked = IIf(dgv_lista.CurrentRow.Cells("MA_ESTADO").Value = 1, True, False)
