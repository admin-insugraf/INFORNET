Public Class frm_AF_MA_Ubicacion
    Dim bol_nuevo As Boolean = False
    Private Sub frm_AF_MA_Ubicacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call Cargar_Datos()
        Inicializa_Tool(ToolS_Mantenimiento)
    End Sub

    Private Sub Cargar_Datos()
        Dim cateBL As New ClsOperaciones.AF_MA_UBICACION
        dgv_lista.DataSource = cateBL.get_ubicaciones
        cateBL = Nothing
    End Sub
    ...
    Private Sub Tool_Grabar_Click(sender As Object, e As EventArgs) Handles Tool_Grabar.Click
        ...
        Dim catBE As New ClsEntidades.AF_MA_UBICACION
        Dim catBL As New ClsOperaciones.AF_MA_UBICACION

        With catBE
            .UB_ID = txt_cod.Text.Trim
            .UB_DESCRIPCION = txt_des.Text.Trim
            .UB_ESTADO = IIf(chk_estado.Checked, 1, 0)
        End With
    ...
    Private Sub Tool_Editar_Click(sender As Object, e As EventArgs) Handles Tool_Editar.Click
        ...
        Cambiar_Estado_Tool(ToolS_Mantenimiento)
        txt_cod.Text = dgv_lista.CurrentRow.Cells("UB_ID").Value
        txt_des.Text = dgv_lista.CurrentRow.Cells("UB_DESCRIPCION").Value
        chk_estado.Checked = IIf(dgv_lista.CurrentRow.Cells("UB_ESTADO").Value = 1, True, False)
    ...
    Private Sub Tool_Eliminar_Click(sender As Object, e As EventArgs) Handles Tool_Eliminar.Click
        ...
        If MsgBox("Seguro de Eliminar?", MsgBoxStyle.Question, "Cuidado!") = MsgBoxResult.Yes Then
            ...
            Dim catBL As New ClsOperaciones.AF_MA_UBICACION
            With catBE
                .UB_ID = dgv_lista.CurrentRow.Cells("UB_ID").Value
            End With
            catBL.delete(catBE)
            catBE = Nothing
            catBL = Nothing
