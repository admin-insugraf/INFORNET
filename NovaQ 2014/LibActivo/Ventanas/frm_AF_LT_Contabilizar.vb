...
Public Class frm_AF_LT_Contabilizar

    Private Sub frm_AF_LT_Contabilizar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Cargar_Subdiario()
        Dim su As New ClsOperaciones.AF_MA_ACTIVO
    ...
    Private Sub Tool_Procesar_Click(sender As Object, e As EventArgs) Handles Tool_Procesar.Click
        If mtb_fec_proc.Text = "" Then ' cmb_mes.SelectedIndex = 0 Then
            MsgBox("Digite una fecha a generar el Asiento de Depreciación", MsgBoxStyle.Exclamation, "Sistemas")
            mtb_fec_proc.Focus()
            Exit Sub
        End If
        ...
        Dim dt_tmp As DataTable = activoBL.Procesar_Asiento_Contable(CDate(mtb_fec_proc.Text).Year, CDate(mtb_fec_proc.Text).Month)
        Dim total As Double = 0
        'Dim f As DataRow

        'If dt_tmp.Rows.Count > 0 Then
        '    total = dt_tmp.Compute("sum(HABER)", "")
        '    f = dt_tmp.NewRow()
        '    f.Item("CUENTA_CONTABLE") = "681"
        '    f.Item("CUENTA") = "DEPRECIACION"
        '    f.Item("DEBE") = total
        '    f.Item("HABER") = 0
        '    dt_tmp.Rows.Add(f)
        'End If
    ...
    Private Function Verificar_Apertura_Cierre_Modulos(ByVal Mes As String, ByVal anyo As String) As Boolean
        ...
        Try
            ...
            Dim VentasBl As ClsOperaciones.AF_MA_ACTIVO
            VentasBl = New ClsOperaciones.AF_MA_ACTIVO
            Dim dtCierre As New DataTable
            dtCierre = VentasBl.Validar_Cierre_Apertura_Modulos(Mes_Ant, Anio_Ant, "ACTIVO_FIJO")
            If dtCierre.Rows.Count() > 0 Then
                MsgBox("El mes seleccionado se encuentra CERRADO," & Chr(13) & "APERTURAR para generar la Depreciación.", MsgBoxStyle.Critical)
                Estado = False
                Exit Try
            End If
    ...
    Private Sub Tool_Generar_Click(sender As Object, e As EventArgs) Handles Tool_Generar.Click
        ...
        If MessageBox.Show("¿Desea generar el Comprobante de la Depreciación?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
    ...
    Private Function IngresarDetallesAsientoContable(ByVal _TipoCambio As Double, ByVal _num_voucher As String) As Boolean
        ...
        Try
            ...
            For I As Integer = 0 To dgv_vista.Rows.Count - 1
                ...
                d.AD_TANEXO = 0
                d.AD_IDANEXO = LibComunVar.ClsVarComun.RucEmpresa
                d.AD_TDOC = "DEPRE"
                d.AD_SDOC = ""
                d.AD_NDOC = Format(CDate(mtb_fec_proc.Text).Month, "00") & CDate(mtb_fec_proc.Text).Year
                d.AD_FDOC = mtb_fec_proc.Text
                d.AD_VDOC = ""
