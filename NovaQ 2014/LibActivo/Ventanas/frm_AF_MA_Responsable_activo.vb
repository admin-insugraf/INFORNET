Public Class frm_AF_MA_Responsable_activo
    Dim bol_nuevo As Boolean = False

    Private Sub frm_AF_MA_Responsable_activo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call Cargar_Datos()
        Inicializa_Tool(ToolS_Mantenimiento)
    End Sub

    Private Sub Cargar_Datos()
        Dim cateBL As New ClsOperaciones.AF_MA_RESPO_ACTI
        dgv_lista.DataSource = cateBL.get_responsables()
        cateBL = Nothing
    End Sub

    Private Sub Tool_Nuevo_Click(sender As Object, e As EventArgs) Handles Tool_Nuevo.Click
        tc_maestro.SelectedIndex = 1
    ...
    Private Sub Tool_Grabar_Click(sender As Object, e As EventArgs) Handles Tool_Grabar.Click
        ...
        Dim catBE As New ClsEntidades.AF_MA_RESPO_ACTI
        Dim catBL As New ClsOperaciones.AF_MA_RESPO_ACTI

        With catBE
            .RA_ID = txt_cod.Text.Trim
            .RA_DESCRIPCION = txt_des.Text.Trim
            .RA_ESTADO = IIf(chk_estado.Checked, 1, 0)
        End With
    ...
    Private Sub Tool_Editar_Click(sender As Object, e As EventArgs) Handles Tool_Editar.Click
        ...
        Cambiar_Estado_Tool(ToolS_Mantenimiento)
        txt_cod.Text = dgv_lista.CurrentRow.Cells("RA_ID").Value
        txt_des.Text = dgv_lista.CurrentRow.Cells("RA_DESCRIPCION").Value
        chk_estado.Checked = IIf(dgv_lista.CurrentRow.Cells("RA_ESTADO").Value = 1, True, False)
        tc_maestro.SelectedIndex = 1
    ...
    Private Sub Tool_Eliminar_Click(sender As Object, e As EventArgs) Handles Tool_Eliminar.Click
        ...
        If MsgBox("Seguro de Eliminar?", MsgBoxStyle.Question, "Cuidado!") = MsgBoxResult.Yes Then
            ...
            Dim catBL As New ClsOperaciones.AF_MA_RESPO_ACTI
            With catBE
                .RA_ID = dgv_lista.CurrentRow.Cells("RA_ID").Value
            End With
            catBL.delete(catBE)
            catBE = Nothing
            catBL = Nothing
