Public Class Frm_Orden_Servicio
    ...
    Private Sub DocumentosOrdenCompra()
        Try
            ...
            dtv = New DataView
            dtDocumentos = OrdenCompraBl.Mostrar_Documentos_Principal(sTipoOrden, dtpfechaInicial.Value.ToString("dd/MM/yyyy"), CDate(dtpfechafinal.Value.ToString("dd/MM/yyyy")), IIf(cboopcionesBusqueda.SelectedIndex = 0, "TODOS", "RANGO"))
    ...
    Private Function Numeracion() As String
        ...
        Try
            CORRELATIVOBL = New ClsTransacciones.PURCHASE_ORDER
            CORRELATIVOBL.Get_NumeroCorrelativoOrdenCompraII("OS")
            If String.IsNullOrEmpty(CORRELATIVOBL.NumeroCorrelativo.ToString) Then
                Correlativo = String.Format("{0:0000000000000}", 1)
            Else
                Correlativo = String.Format("{0:0000000000000}", CInt(CORRELATIVOBL.NumeroCorrelativo + 1))
    ...
    Private Function Guardar_Orden_Compra() As Boolean
        Try
            ...
            Next
            If OrdenCompraBl.GuardarOrdenServicio(OrdenCompraCabENT, ListaDetalles, _TipoAnexoDoc, Modo_consultar) Then
                If Modo_consultar = True Then
                ...
                End If
                OrdenCompraBl = New ClsOperaciones.PURCHASE_ORDER
                OrdenCompraBl.ActualizandoEstadoOrdenServicio(ListaDetalles)
                If MessageBox.Show("Se procedera a la Impresion de Documentos" & vbCrLf & "¿Desea continuar?", "Imprimir", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    imprimir_Orden_Servicio(OrdenCompraCabENT.ID, OrdenCompraCabENT.AMOUNT_SALES, OrdenCompraCabENT.CURRENCY_TYPE)
                End If
                Return True
            Else
                Return False
    ...
    Private Sub btnGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        If Modo_consultar = True Then
            mensaje = "Se va a modificar la Orden de Servicio."
        Else
            mensaje = "Se va a Generar la Orden de Servicio."
        End If
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        Try
            ...
            If estadoInicial = "05" Then
                ...
                Exit Sub
            End If
            If MessageBox.Show("Se va a cambiar el estado de la Orden de Servicio" & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                Select Case estadoActualizado
                    Case "01"
                        If MessageBox.Show("Confirmacion de cambio de estado a : EMITIDA " & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                            sql = "UPDATE PURCHASE_ORDER SET STATUS_ID='" & estadoActualizado & "' WHERE ID='" & _id & "' and TYPE='OS'"
    ...
    Private Sub imprimir_Orden_Servicio(ByVal _Codigo As String, ByVal _Monto As Double, ByVal _Moneda As String)
        Try
            ...
            End If
            dtImprimir = reporteBL.EjecutarReporteOrdenServicio(_Codigo)
            If dtImprimir.Rows.Count() <> 0 Then
                'crystalBL.Muestra_Reporte("rpt_Mov_Almacen.rpt", "", "", "", "@almacen;" & _idalamacen, "@tipo;" & _documentid, "@numero;" & _numerodocument)
                crystalBL.Muestra_Reporte("rpt_Orden_Servicio.rpt", "", "", "", "@NUMERO;" & _Codigo, "@TIPO;" & "OS", "@LETRA;" & STRmontotexto)
            End If
    ...
    Private Function Verificar_Apertura_Cierre_Modulos(ByVal Mes As String, ByVal anyo As String) As Boolean
        ...
        Try
            ...
            If dtCierre.Rows.Count() > 0 Then
                MsgBox("El mes seleccionado se encuentra CERRADO," & Chr(13) & "APERTURAR para generar la Orden de Servicio.", MsgBoxStyle.Critical)
                Estado = False
                Exit Try
            End If
    ...
    Private Sub AgregarArticuloManual(ByVal _codigoArticulo As String)
        Try
            ...
            dtDocumentoGuias = clsArticuloBl.VerificacionArticuloCodigo(_codigoArticulo, "RS")
    ...
    Private Sub Ayuda_Requisiciones()
        Try
            ...
            dtrequis = OrdenCompraBl.Requisicion_Detalle_Orden_Servicio(ID_REQ, frm_detalles._lista_items)
            'GenerarColummnaDataTable()
    ...
    Private Sub btn_desaprobar_Click(sender As Object, e As EventArgs) Handles btn_desaprobar.Click
        Try
            ...
            If MessageBox.Show("Se va a proceder a RECHAZAR la Orden de Compra." & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Dim sql As String = ""
                sql = "UPDATE PURCHASE_ORDER SET STATUS_ID='06' WHERE ID='" & txtcodigo.Text & "' and TYPE='OS'"
                If OrdenCompraBl.Cambiar_Estado_Oden_Compra(sql) Then
                    MsgBox("Cambio de estado realizado Correctamente.", MsgBoxStyle.Information)
                    btnCancelar.PerformClick()
                End If
