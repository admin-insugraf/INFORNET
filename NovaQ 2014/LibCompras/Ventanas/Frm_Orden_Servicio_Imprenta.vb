Imports LibComunVar
Public Class Frm_Orden_Servicio_Imprenta
    ...
    Dim OrdenCompraDetENT_Lotes As ClsEntidades.PURCHASE_ORDER_LINE_LOTES
    Dim ListaDetalles As List(Of ClsEntidades.PURCHASE_ORDER_LINE)
    Dim ListaDetalles_Lotes As List(Of ClsEntidades.PURCHASE_ORDER_LINE_LOTES)
    ...
    Dim dtdetalleArticuloPrincipal_Lotes As DataTable
    ...
    Private Sub Frm_Orden_Servicio_Imprenta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub GenerarColummnaDataTable_Contenedor()
        dtdetalleArticuloPrincipal_Lotes = New DataTable
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("ITEM_PRODUCTO", Type.GetType("System.Int64"))
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("ITEM_ME", Type.GetType("System.Int64"))
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("ITEM", Type.GetType("System.Int64"))
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("PART_ID_ME", Type.GetType("System.String"))
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("COD_PRODUCTO", Type.GetType("System.String"))
        dtdetalleArticuloPrincipal_Lotes.Columns.Add("REQUI_ID", Type.GetType("System.String"))
    End Sub
    ...
    '        CORRELATIVOBL.Get_NumeroCorrelativoOrdenServicio()
    ...
    Private Function Numeracion() As String
        ...
        Try
            CORRELATIVOBL = New ClsTransacciones.PURCHASE_ORDER
            CORRELATIVOBL.Get_NumeroCorrelativoOrdenCompraII("OI")
            If String.IsNullOrEmpty(CORRELATIVOBL.NumeroCorrelativo.ToString) Then
                Correlativo = String.Format("{0:0000000000000}", 1)
            Else
                Correlativo = String.Format("{0:0000000000000}", CInt(CORRELATIVOBL.NumeroCorrelativo + 1))
    ...
    Private Function ConsultandoDocumentos() As Boolean
        ...
        Try
            ...
            dtCabeceraoC = OrdenCompraBl.Modo_Edicion_Cabecera_Orden_Servicio_Imprenta(_id)
            If dtCabeceraoC.Rows.Count() <> 0 Then
                ...
                Else
                    gbOpciones.Enabled = True
                End If
                If dtCabeceraoC.Rows(0).Item("FLAG_PROMISE_DATE").ToString = "0" Then
                    chkfecha_entrega.Checked = False
                Else
                    chkfecha_entrega.Checked = True
                End If
                ...
                OrdenCompraBl = New ClsOperaciones.PURCHASE_ORDER
                ' GenerarColummnaDataTable()
                dtdetalleArticuloPrincipal = OrdenCompraBl.Modo_Edicion_Detalle_Orden_Servicio_Imprenta(_id)
                dtdetalleArticuloPrincipal_Lotes = OrdenCompraBl.Modo_Edicion_Detalle_Orden_Servicio_Imprenta_Lotes(_id)
    ...
    '        OrdenCompraBl = New ClsOperaciones.PURCHASE_ORDER
    '        Dim dtCabeceraoC As New DataTable
    '        dtCabeceraoC = OrdenCompraBl.Modo_Edicion_Cabecera_Orden_Servicio(_id)
    '        If dtCabeceraoC.Rows.Count() <> 0 Then
    '            txtcodigo.Text = dtCabeceraoC.Rows(0).Item(0).ToString
    '            dtpemision.Value = dtCabeceraoC.Rows(0).Item("DOCUMENT_DATE").ToString
    '            txtestado.Text = dtCabeceraoC.Rows(0).Item("STATUS_ID").ToString
    ...
    '            OrdenCompraBl = New ClsOperaciones.PURCHASE_ORDER
    '            ' GenerarColummnaDataTable()
    '            dtdetalleArticuloPrincipal = OrdenCompraBl.Modo_Edicion_Detalle_Orden_Servicio(_id)
    '            If dtdetalleArticuloPrincipal.Rows.Count() <> 0 Then
    '                ConfigurarGrila()
    '                CalcularTotales()
    '            End If
    ...
    Private Sub Guardar_Orden_Compra()
        Try
            ...
            With OrdenCompraCabENT
                ...
                .FLAG_FLETE_IGV = IIf(chkflesinigv.Checked, 0, 1)
            End With
            For Each item As DataRow In dtdetalleArticuloPrincipal.Rows
                ...
                With OrdenCompraDetENT
                    ...
                    .TYPE = "OI"
                    .ITEM = item("ITEM")
                    .PART_ID = item("PART_ID_ME")
                    .PART_DESCRIPTION = item("DESCRIPTION_ME")
                    .UNIT = item("UNIT_ME")
                    ...
                    .PROJECT_ID = IIf(String.IsNullOrEmpty(item("COD_PRODUCTO").ToString), "", item("COD_PRODUCTO"))
                    ...
                    .FLAG_FREE_IGV = IIf(String.IsNullOrEmpty(item("FLAG_FREE_IGV").ToString), 0, item("FLAG_FREE_IGV"))
                    'If _TipoAnexoDoc = "COTIZACION" Then
                    '    .COTI_ID = _NumeroTipoAnexoDoc
                    'ElseIf _TipoAnexoDoc = "REQUISICION" Then
                    '    .REQUI_ID = _NumeroTipoAnexoDoc
                    'End If
                End With
                ListaDetalles.Add(OrdenCompraDetENT)
            Next
            'detalle de lotes
            For Each item As DataRow In dtdetalleArticuloPrincipal_Lotes.Rows
                OrdenCompraDetENT_Lotes = New ClsEntidades.PURCHASE_ORDER_LINE_LOTES
                With OrdenCompraDetENT_Lotes
                    .ID = OrdenCompraCabENT.ID
                    .TYPE = "OI"
                    .ITEM_PRODUCTO = item("ITEM_PRODUCTO")
                    .ITEM_ME = item("ITEM_ME")
                    .ITEM = item("ITEM")
                    .PART_ID_ME = item("PART_ID_ME")
                    .COD_PRODUCTO = item("COD_PRODUCTO")
                    .REQUI_ID = item("REQUI_ID")
                End With
                ListaDetalles_Lotes.Add(OrdenCompraDetENT_Lotes)
            Next
            If OrdenCompraBl.GuardarOrdenServicio_Imprenta(OrdenCompraCabENT, ListaDetalles, _TipoAnexoDoc, Modo_consultar, ListaDetalles_Lotes) Then
                If Modo_consultar = True Then
                ...
                End If
                ' OrdenCompraBl = New ClsOperaciones.PURCHASE_ORDER
                ' OrdenCompraBl.ActualizandoEstadoOrdenServicio_Imprenta(ListaDetalles)
                If MessageBox.Show("Se procedera a la Impresion de Documentos" & vbCrLf & "¿Desea continuar?", "Imprimir", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    imprimir_Orden_Servicio(OrdenCompraCabENT.ID, OrdenCompraCabENT.AMOUNT_SALES, IIf(OrdenCompraCabENT.CURRENCY_TYPE = "MN", "MN", "ME"))
                End If
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub btnGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        If Modo_consultar = True Then
            mensaje = "Se va a modificar la Orden de Servicio Imprenta."
        Else
            mensaje = "Se va a Generar la Orden de Servicio Imprenta."
        End If
        If MessageBox.Show(mensaje & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            If dtdetalleArticuloPrincipal.Rows.Count() <> 0 Then
                ...
                For Each row As DataRow In dtdetalleArticuloPrincipal.Select("QTY_ME=0 OR PRICE=0", "")
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        Try
            ...
            If estadoInicial = "05" Then
                ...
                Exit Sub
            End If
            If MessageBox.Show("Se va a cambiar el estado de la Orden de Servicio Imprenta" & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                Select Case estadoActualizado
                    Case "01"
                        If MessageBox.Show("Confirmacion de cambio de estado a : EMITIDA " & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                            sql = "UPDATE PURCHASE_ORDER SET STATUS_ID='" & estadoActualizado & "' WHERE ID='" & _id & "' and TYPE='OI'"
                    ...
                    Case "02"
                        ...
                        End If
                        'Case "05"
                        '    If MessageBox.Show("Se va a anular la Orden de Servicio" & vbCrLf & "¿Desea continuar?", "Anular", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                        '        If OrdenCompraBl.Anular_Oden_Servicio(_id) Then
    ...
    Private Sub imprimir_Orden_Compra(ByVal _Codigo As String, ByVal _Monto As Double, ByVal _Moneda As String)
        Try
            ...
            If dtImprimir.Rows.Count() <> 0 Then
                ...
                crystalBL.Muestra_Reporte("rpt_Orden_Compra.rpt", "", "", "", "@NUMERO;" & _Codigo, "@TIPO;" & "OI", "@LETRA;" & STRmontotexto)
            End If
    ...
    Private Sub imprimir_Orden_Servicio(ByVal _Codigo As String, ByVal _Monto As Double, ByVal _Moneda As String)
        Try
            reporteBL = New ClsBuscar
            crystalBL = New LibReportes.ClsReporte
            dtImprimir = New DataTable("OrdenCompra")
            ...
            End If
            dtImprimir = reporteBL.EjecutarReporteOrdenServicio_Imprenta(_Codigo)
            If dtImprimir.Rows.Count() <> 0 Then
                'crystalBL.Muestra_Reporte("rpt_Mov_Almacen.rpt", "", "", "", "@almacen;" & _idalamacen, "@tipo;" & _documentid, "@numero;" & _numerodocument)
                crystalBL.Muestra_Reporte("rpt_Orden_Servicio_Imprenta.rpt", "", "", "", "@NUMERO;" & _Codigo, "@TIPO;" & "OI", "MONTO_LETRAS;" & STRmontotexto)
            End If
            'dtImprimir = reporteBL.BuscarRequisiciones_OrdenServicio(_Codigo)
            'If dtImprimir.Rows.Count() <> 0 Then
            '    For Each row As DataRow In dtImprimir.Rows
            '        dtImprimir = reporteBL.EjecutarReporteRequisiciones("RS", row("REQUI_ID"))
            '        If dtImprimir.Rows.Count() <> 0 Then
            '            crystalBL.Muestra_Reporte("rpt_Compras_requisiciones.rpt", "", "", "", "@TYPE;" & "RS", "@ID;" & row("REQUI_ID"))
            '        End If
            '    Next
            'End If
            ...
            '    If dtImprimir.Rows.Count() <> 0 Then
            '        For Each row As DataRow In dtImprimir.Rows
            '            dtImprimir = reporteBL.EjecutarReporteRequisiciones("RI", row("REQUI_ID"))
            '            If dtImprimir.Rows.Count() <> 0 Then
            '                crystalBL.Muestra_Reporte("rpt_Compras_requisiciones.rpt", "", "", "", "@TYPE;" & "RI", "@ID;" & row("REQUI_ID"))
            '            End If
            '        Next
            '    End If
            'End If
    ...
    Private Sub btnImprimir_Click(sender As Object, e As EventArgs) Handles btnImprimir.Click
        ...
        ' _Id = dgvCabecera.Item(2, dgvCabecera.CurrentRow.Index).Value
        ' _Monto = IIf(String.IsNullOrEmpty(dgvCabecera.Item(8, dgvCabecera.CurrentRow.Index).Value.ToString), 0, dgvCabecera.Item(8, dgvCabecera.CurrentRow.Index).Value)
        '_Moneda = dgvCabecera.Item(9, dgvCabecera.CurrentRow.Index).Value
        ...
        imprimir_Orden_Servicio(_Id, _Monto, _Moneda)
    End Sub
    ...
    Private Sub btnModificar_Click(sender As Object, e As EventArgs) Handles btnModificar.Click
        Try
            ...
            For Each row As DataRow In dtdetalleArticuloPrincipal.Select("ITEM=" & _Codigo_Ini)
                txtcodigoArticulo.Text = row("PART_ID_ME")
                txtdescripcionArticulo.Text = row("DESCRIPTION_ME")
                txtunidadMedida.Text = row("UNIT_ME")
                txtunidadMedRef.Text = row("UNIT_REFERENCE")
                txtcantidad.Text = CDbl(row("QTY_ME"))
                txtcantidadref.Text = CDbl(row("NUM_LOTES"))
    ...
    Private Sub EliminandoArticulo()
        Try
            If MessageBox.Show("Se va a eliminar el registro" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                For Each item As DataRow In dtdetalleArticuloPrincipal_Lotes.Select("ITEM=" & Codigo)
                    dtdetalleArticuloPrincipal_Lotes.BeginInit()
                    item.Delete()
                    dtdetalleArticuloPrincipal_Lotes.EndInit()
                    dtdetalleArticuloPrincipal_Lotes.AcceptChanges()
                Next
        ...
        Try
            ...
            If Val(txtValorunitario.Text) <> 0 Then  'Valor unitario
                If chk_libre.Checked Then
                    ...
                    nValorVta = nValorUnit * Val(txtcantidad.Text) * Val(txtcantidadref.Text)
                    nMontodes = nDsctoUnit * Val(txtcantidad.Text) * Val(txtcantidadref.Text)
                    nMontoIgv = 0
                    nMontoTotal = nValorVta + nMontoIgv
                    nValorBruto = nValorVta + nMontodes
                    txttasa.Text = 0
                Else
                    ...
                    End If
                    If Val(txtcantidadref.Text) <> 0 And Val(txtcantidad.Text) <> 0 Then
                        '    nValorUniRef = 0 'Math.Round(Val(txtcantidadref.Text) * Val(txtcantidad.Text) * nValorUnit, 6) 'Math.Round((Val(txtcantidadref.Text) / Val(txtcantidad.Text)) * nValorUnit, 6)
                        '    nDsctoUnit = nValorUniRef * (IIf(txtporc_descuento.Text = "", 0, txtporc_descuento.Text)) / 100
                        '    nValorUniRef = nValorUniRef - nDsctoUnit
                        '    nValorUnit = nValorUniRef
                        'Else
                    ...
                    nValorVta = nValorUnit * Val(txtcantidad.Text) * Val(txtcantidadref.Text)
                    nMontodes = nDsctoUnit * Val(txtcantidad.Text) * Val(txtcantidadref.Text)
                    nMontoIgv = (nValorUnit * nTasaIGV) * Val(txtcantidad.Text) * Val(txtcantidadref.Text)
                    nMontoTotal = nValorVta + nMontoIgv
                    nValorBruto = nValorVta + nMontodes
                End If
            End If
    ...
    Private Sub btnAceptar_Click(sender As Object, e As EventArgs) Handles btnAceptar.Click
        Try
            ...
            Else
                For Each row As DataRow In dtdetalleArticuloPrincipal.Select("ITEM=" & _Codigo_Ini)
                    dtdetalleArticuloPrincipal.BeginInit()
                    row("PART_ID_ME") = txtcodigoArticulo.Text
                    row("DESCRIPTION_ME") = txtdescripcionArticulo.Text
                    row("UNIT_ME") = txtunidadMedida.Text
                    row("UNIT_REFERENCE") = txtunidadMedRef.Text
                    row("QTY_ME") = IIf(txtcantidad.Text = "", 0, txtcantidad.Text)
                    row("FACTOR_REFERENCE") = IIf(txtcantidadref.Text = "", 0, txtcantidadref.Text)
                    row("PRICE") = IIf(txtValorunitario.Text = "", 0, txtValorunitario.Text)
                    row("DISCOUNT_PER") = IIf(txtporc_descuento.Text = "", 0, txtporc_descuento.Text)
                    row("DISCOUNT") = IIf(txtdescuento.Text = "", 0, txtdescuento.Text)
    ...
    Private Sub AgregarArticuloManual(ByVal _codigoArticulo As String)
        Try
            ...
            dtDocumentoGuias = clsArticuloBl.VerificacionArticuloCodigo(_codigoArticulo, "RI")
            ...
            If dtDocumentoGuias.Rows.Count() <> 0 Then
                ...
                End If
            Else
                MsgBox("El codigo ingresado no pertence a la Familia lusa.", MsgBoxStyle.Critical)
    ...
    Private Sub txtcodigo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txttipoCambio.KeyPress, txtSolicitante.KeyPress, txtrucfacturar.KeyPress, txtproveedor_ruc.KeyPress, txtproveedor_razons.KeyPress, txtproveedor.KeyPress, txtnombreFacturar.KeyPress, txtFormaPago.KeyPress, txtentrega.KeyPress, txtdireccionFacturar.KeyPress, txtcotizacion.KeyPress, txtcodigo.KeyPress, dtpEntrega.KeyPress, dtpemision.KeyPress, cbomoneda.KeyPress
    ...
    Private Sub RehacerSecuencia()
        ...
        Next
        i = 0
        For Each items As DataRow In dtdetalleArticuloPrincipal_Lotes.Rows
            i += 1
            dtdetalleArticuloPrincipal_Lotes.BeginInit()
            items("ITEM") = i
            dtdetalleArticuloPrincipal_Lotes.EndInit()
    ...
    Private Sub Ayuda_Requisiciones()
        Try
            ...
            frm._Flag_Filtro = True
            frm.Filtros1 = "RI"
            frm.Titulo = "LISTA DE SERVICIOS DE IMPRENTA"
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
                ID_REQ = frm.Data_Matriz.Rows(0).Item(0).ToString
                'frm_detalles._numero_requisicion = ID_REQ
                'frm_detalles._tipo_requisicion = "RI"
                'frm_detalles.Titulo = "DETALLE DE REQUISICION DE IMPRENTA N° " & ID_REQ
            ...
            Dim dtrequis_lotes As New DataTable
            dtrequis = OrdenCompraBl.Requisicion_Detalle_Orden_Servicio_Imprenta(ID_REQ, frm_detalles._lista_items)
            ...
            For Each ROW In dtrequis.Rows
                Dim FILA As DataRow = dtdetalleArticuloPrincipal.NewRow
                FILA("ITEM") = dtdetalleArticuloPrincipal.Rows.Count + 1
                FILA("ITEM_PRODUCTO") = ROW("ITEM_PRODUCTO")
                FILA("ITEM_ME") = ROW("ITEM_ME")
                FILA("PART_ID_ME") = ROW("PART_ID_ME")
                FILA("DESCRIPTION_ME") = ROW("DESCRIPTION_ME")
                FILA("UNIT_ME") = ROW("UNIT_ME")
                FILA("UNIT_REFERENCE") = ROW("UNIT_REFERENCE")
                FILA("FACTOR_REFERENCE") = ROW("FACTOR_REFERENCE")
                FILA("QTY_ME") = ROW("QTY_ME")
                FILA("PRICE") = ROW("PRICE")
                FILA("DISCOUNT_PER") = ROW("DISCOUNT_PER")
                FILA("DISCOUNT") = ROW("DISCOUNT")
                FILA("AMOUNT_TAX") = ROW("AMOUNT_TAX")
                FILA("PERCENT_TAX") = ROW("PERCENT_TAX")
                FILA("PRICE_NET") = ROW("PRICE_NET")
                FILA("AMOUNT_TOTAL") = ROW("AMOUNT_TOTAL")
                FILA("AMOUNT_TOTAL_NET") = ROW("AMOUNT_TOTAL_NET")
                FILA("QTY_OUT") = ROW("QTY_OUT")
                FILA("STATUS_ID") = ROW("STATUS_ID")
                FILA("COMMENT1") = ROW("COMMENT1")
                FILA("TEXT_COMMENT") = ROW("TEXT_COMMENT")
                FILA("IS_VAT") = ROW("IS_VAT")
                FILA("PRICE_UNIT") = ROW("PRICE_UNIT")
                FILA("CCOST_ID") = ROW("CCOST_ID")
                FILA("PROJECT_ID") = ROW("PROJECT_ID")
                FILA("AREA_ID") = ROW("AREA_ID")
                FILA("COTI_ID") = ROW("COTI_ID")
                FILA("REQUI_ID") = ROW("REQUI_ID")
                FILA("COD_PRODUCTO") = ROW("COD_PRODUCTO")
                FILA("PRODUCTO") = ROW("PRODUCTO")
                FILA("NUM_LOTES") = ROW("NUM_LOTES")

                Dim FILA2 As DataRow = dtdetalleArticuloPrincipal_Lotes.NewRow
                FILA2("ITEM_PRODUCTO") = ROW("ITEM_PRODUCTO")
                FILA2("ITEM_ME") = ROW("ITEM_ME")
                FILA2("ITEM") = dtdetalleArticuloPrincipal.Rows.Count + 1
                FILA2("PART_ID_ME") = ROW("PART_ID_ME")
                FILA2("COD_PRODUCTO") = ROW("COD_PRODUCTO")
                FILA2("REQUI_ID") = ROW("REQUI_ID")

                dtdetalleArticuloPrincipal.Rows.Add(FILA)
                dtdetalleArticuloPrincipal.AcceptChanges()

                
                dtdetalleArticuloPrincipal_Lotes.Rows.Add(FILA2)
                dtdetalleArticuloPrincipal_Lotes.AcceptChanges()
            Next ROW
            ...
            _TipoAnexoDoc = "REQUISICION IMPRENTA"
        Catch ex As Exception
            GenerarColummnaDataTable()
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub dgvCabecera_CurrentCellDirtyStateChanged(sender As Object, e As EventArgs)
    ...
    Private Sub btn_anular_Click(sender As Object, e As EventArgs) Handles btn_anular.Click
        ...
        If estadoInicial = "05" Then
            ...
            Exit Sub
        End If
        If MessageBox.Show("Se va a anular la Orden de Imprenta" & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            If OrdenCompraBl.Anular_Oden_ServicioImprenta(_id) Then
                MsgBox("Documento anulado Correctamente.", MsgBoxStyle.Information)
                DocumentosOrdenCompra()
    ...
    Private Sub btn_aprobar_Click(sender As Object, e As EventArgs) Handles btn_aprobar.Click
        Try
            ...
            If MessageBox.Show("Se va a proceder a APROBAR la Orden de Compra." & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Dim sql As String = ""
                sql = "UPDATE PURCHASE_ORDER SET STATUS_ID='02' WHERE ID='" & txtcodigo.Text & "' and TYPE='OS'"
                If OrdenCompraBl.Cambiar_Estado_Oden_Compra(sql) Then
                    MsgBox("Cambio de estado realizado Correctamente.", MsgBoxStyle.Information)
                    btnCancelar.PerformClick()
                End If
    ...
    Private Sub btn_desaprobar_Click(sender As Object, e As EventArgs) Handles btn_desaprobar.Click
        Try
            ...
            If MessageBox.Show("Se va a proceder a RECHAZAR la Orden de Compra." & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Dim sql As String = ""
                sql = "UPDATE PURCHASE_ORDER SET STATUS_ID='06' WHERE ID='" & txtcodigo.Text & "' and TYPE='OI'"
                If OrdenCompraBl.Cambiar_Estado_Oden_Compra(sql) Then
                    MsgBox("Cambio de estado realizado Correctamente.", MsgBoxStyle.Information)
                    btnCancelar.PerformClick()
                End If
    ...
    Public Sub GenerarColummnaDataTable()
        Try
            ...
            dtdetalleArticuloPrincipal.Columns.Add("ITEM_PRODUCTO", Type.GetType("System.Int64")) '1
            dtdetalleArticuloPrincipal.Columns.Add("ITEM_ME", Type.GetType("System.Int64")) '2
            dtdetalleArticuloPrincipal.Columns.Add("PART_ID_ME", Type.GetType("System.String")) '3
            dtdetalleArticuloPrincipal.Columns.Add("DESCRIPTION_ME", Type.GetType("System.String")) '4
            dtdetalleArticuloPrincipal.Columns.Add("UNIT_ME", Type.GetType("System.String")) '5
            dtdetalleArticuloPrincipal.Columns.Add("UNIT_REFERENCE", Type.GetType("System.String")) '6
            dtdetalleArticuloPrincipal.Columns.Add("FACTOR_REFERENCE", Type.GetType("System.Double")) '7
            dtdetalleArticuloPrincipal.Columns.Add("QTY_ME", Type.GetType("System.Double")) '8
            dtdetalleArticuloPrincipal.Columns.Add("PRICE", Type.GetType("System.Double")) '9
            dtdetalleArticuloPrincipal.Columns.Add("DISCOUNT_PER", Type.GetType("System.Double")) '10'
            dtdetalleArticuloPrincipal.Columns.Add("DISCOUNT", Type.GetType("System.Double")) '11
            dtdetalleArticuloPrincipal.Columns.Add("AMOUNT_TAX", Type.GetType("System.Double")) '12'
            dtdetalleArticuloPrincipal.Columns.Add("PERCENT_TAX", Type.GetType("System.Double")) '13
            dtdetalleArticuloPrincipal.Columns.Add("PRICE_NET", Type.GetType("System.Double")) '14'
            dtdetalleArticuloPrincipal.Columns.Add("AMOUNT_TOTAL", Type.GetType("System.Double")) '15
            dtdetalleArticuloPrincipal.Columns.Add("AMOUNT_TOTAL_NET", Type.GetType("System.Double")) '16
            dtdetalleArticuloPrincipal.Columns.Add("QTY_ENTER", Type.GetType("System.Double")) '17
            dtdetalleArticuloPrincipal.Columns.Add("QTY_OUT", Type.GetType("System.Double")) '18
            dtdetalleArticuloPrincipal.Columns.Add("STATUS_ID", Type.GetType("System.String")) '19
            dtdetalleArticuloPrincipal.Columns.Add("COMMENT1", Type.GetType("System.String")) '20
            dtdetalleArticuloPrincipal.Columns.Add("TEXT_COMMENT", Type.GetType("System.String")) '21
            dtdetalleArticuloPrincipal.Columns.Add("IS_VAT", Type.GetType("System.String")) '22
            dtdetalleArticuloPrincipal.Columns.Add("PRICE_UNIT", Type.GetType("System.Double")) '23
            dtdetalleArticuloPrincipal.Columns.Add("CCOST_ID", Type.GetType("System.String")) '24
            dtdetalleArticuloPrincipal.Columns.Add("PROJECT_ID", Type.GetType("System.String")) '25
            dtdetalleArticuloPrincipal.Columns.Add("AREA_ID", Type.GetType("System.String")) '26
            dtdetalleArticuloPrincipal.Columns.Add("COTI_ID", Type.GetType("System.String")) '27
            dtdetalleArticuloPrincipal.Columns.Add("REQUI_ID", Type.GetType("System.String")) '28
            dtdetalleArticuloPrincipal.Columns.Add("FLAG_FREE_IGV", Type.GetType("System.Int64")) '29
            dtdetalleArticuloPrincipal.Columns.Add("COD_PRODUCTO", Type.GetType("System.String")) '30
            dtdetalleArticuloPrincipal.Columns.Add("PRODUCTO", Type.GetType("System.String")) '31
            dtdetalleArticuloPrincipal.Columns.Add("NUM_LOTES", Type.GetType("System.Int64")) '32
    ...
    Private Sub ConfigurarGrila()
        If dtdetalleArticuloPrincipal.Rows.Count() <> 0 Then
            ...
            dgvDetalle.Columns(3).HeaderText = "Codigo"
            dgvDetalle.Columns(4).HeaderText = "Producto"
            dgvDetalle.Columns(5).HeaderText = "Unidad"
            ...
            dgvDetalle.Columns(14).HeaderText = "Valor.Venta"
            dgvDetalle.Columns(15).Visible = False
            dgvDetalle.Columns(16).HeaderText = "Total"

            For i As Integer = 17 To 26
                dgvDetalle.Columns(i).Visible = False
            Next

            dgvDetalle.Columns(27).HeaderText = "Nro.Cotizacion"
            dgvDetalle.Columns(28).HeaderText = "Nro.Requisicion"
            dgvDetalle.Columns(29).Visible = False
            dgvDetalle.Columns(30).HeaderText = "Cod. Producto"
            dgvDetalle.Columns(31).HeaderText = "Producto"
            dgvDetalle.Columns(32).HeaderText = "Nro. de Lotes"
    ...
    Private Sub DocumentosOrdenCompra()
        Try
            ...
            dtv = New DataView
            dtDocumentos = OrdenCompraBl.Mostrar_Documentos_Principal(sTipoOrden, dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value.ToString("dd/MM/yyyy"), IIf(cboopcionesBusqueda.SelectedIndex = 0, "TODOS", "RANGO"))
