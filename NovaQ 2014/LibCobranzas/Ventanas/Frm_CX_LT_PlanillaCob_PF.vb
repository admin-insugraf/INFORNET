...
Public Class Frm_CX_LT_PlanillaCob_PF
    ...
    Dim STRorden_det As String = String.Empty
    Dim STRorden_det_reporte As String = String.Empty
    ...
    Private Sub Frm_CX_LT_PlanillaCob_PF_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "subprocesos"
    ...
    Private Sub Cargar_Tipo_Cobranza()
        ...
        cmb_tipo_cob.DataSource = tipoCobranzaBL.get_Tipos_PF(GRUPO_COBRANZA)
    ...
    Private Sub Cargar_Documentos_Pendientes_por_fechas(ByVal _fecha_ini As String, ByVal _fecha_fin As String)
        Dim saldosBL As New ClsOperaciones.CUSTOMER_BALANCE
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes_x_Fechas(_fecha_ini, _fecha_fin)
        'dgv_pendientes.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'dgv_pendientes.AutoResizeColumns()
        'dgv_pendientes.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        saldosBL = Nothing
    ...
    Private Sub Consultar_Planillas_Cab_xfecha()
        ...
        dtvC = planillaBL.get_planilla_x_fecha_PF(fecha_ini, fecha_fin, GRUPO_COBRANZA).DefaultView
...
#End Region
    ...
    Private Sub btn_crear_planilla_Click(sender As Object, e As EventArgs) Handles btn_crear_planilla.Click
        ...
        dtTipoCamb_tc = cl.Verifircar_Planilla_Cobros_PF(dtp_fec_nueva_pla.Value.ToString("dd/MM/yyyy"), GRUPO_COBRANZA)
        ...
        End With
        planillaBL.Insert_PF(planillaBE, True, numero_planilla)
        planillaBE = Nothing
        planillaBL = Nothing
    ...
    Private Sub btn_grabar_det_Click(sender As Object, e As EventArgs) Handles btn_grabar_det.Click
        ...
        If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
            If cmb_tipo_doc.SelectedValue = "NC" Then
                ESTADO_GRABADO = planilladetBL.InsertNC_PF(ls_detalle, cmb_tipo_doc.SelectedValue.ToString, txtserieDoc.Text, txtnumDoc.Text, (Math.Abs(CDbl(txtmonto.Text))) * -1, txt_cod_mon.Text, FechaActualPlanilla)
            ...
            Else
                ESTADO_GRABADO = planilladetBL.Insert_PF(ls_detalle, txt_cod_mon.Text, FechaActualPlanilla)
            End If
        Else
            ESTADO_GRABADO = planilladetBL.Insert_PF(ls_detalle, txt_cod_mon.Text, FechaActualPlanilla)
        End If
    ...
    Private Sub Editar_Planilla()
        ...
        dt_detalle = New DataTable
        dtvD = New DataView
        dt_detalle = planillaBL.get_planilla_Det_xID_PF(num_planilla)
        If dt_detalle.Rows.Count() <> 0 Then
            dtvD = dt_detalle.DefaultView
            ...
            STRorden_det = dgv_detalles.Columns(3).Name & "+" & dgv_detalles.Columns(4).Name & "+" & dgv_detalles.Columns(5).Name & "+" & dgv_detalles.Columns(6).Name
            lblCantidadDetall.Text = "Se encontraron " & dtvD.Count & " registros"
        Else
            dgv_detalles.DataSource = Nothing
            STRorden_det = ""
            lblCantidadDetall.Text = "Se encontraron 0 registros"
        End If
    End Sub
            ...
            Else
                ...
                If dt_detalle.Rows.Count() <> 0 Then
                    ...
                    If planillaBL.ActualizacionDocumentosEliminados(CodCliente, tipoDoc, numeroDoc, MontodoC) = True Then
                        ...
                        End If
                        planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE
                        If planillaBL.EliminarDetallesPlanillas_PF(txt_numero_planilla.Text, Item, tipoDoc, numeroDoc) = True Then
    ...
    Private Sub txtbuscar_TextChanged(sender As Object, e As EventArgs) Handles txtbuscar.TextChanged
        If STRorden_det <> "" Then
            Dim wbusqueda As String = UCase(txtbuscar.Text)
            Dim myCurrencyManager As CurrencyManager
            myCurrencyManager = CType(Me.BindingContext(dtvD), CurrencyManager)
            Dim INTnewpos As Integer
            dtvD.RowFilter = STRorden_det & " like '%" & txtbuscar.Text & "%'"
            myCurrencyManager.Position = INTnewpos
    ...
    Private Sub btn_elimianr_planilla_Click(sender As Object, e As EventArgs) Handles btn_elimianr_planilla.Click
        ...
        If MessageBox.Show("Se va a eliminar el registro" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            Else
                ...
                DTRESULTADOS = PlanillaBl.EliminarPlanilla_PF(dgv_planillas.Item(0, dgv_planillas.CurrentRow.Index).Value)
    ...
    Private Sub MostrarDocumentosPendiente_Cliente()
        Try
            ...
            If dtDocumentosPrincipal.Rows.Count() <> 0 Then
                ...
                STRorden_det_reporte = dgvDocumentos.Columns(1).Name & "+" & dgvDocumentos.Columns(2).Name & "+" & dgvDocumentos.Columns(3).Name
            Else
    ...
    Private Sub ImprimirDocumentosPendientes(ByVal _Codigo As String, ByVal _OPCION As Integer)
        Try
            ...
            If optrangocobro.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_PLanilla_Cobranza_PF(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    If optresumido.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_COB_RESUMIDO.rpt", dtImprimir, "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    ElseIf optdetallado.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_COB_DETALLADO.rpt", dtImprimir, "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    End If
                Else
            ...
            ElseIf optexcel.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_Plan_Cob_Cliente_Codigo_PF(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    crystalBL.Muestra_Reporte("REP_PLA_COB_CLIENTE.rpt", dtImprimir, "", "", "@codigo;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                Else
                    MsgBox("No hay informacion disponible para mostrar.", MsgBoxStyle.Information)
                End If
            End If
    ...
    Private Sub txtfiltrocliente_TextChanged_1(sender As Object, e As EventArgs) Handles txtfiltrocliente.TextChanged
        If STRorden_det_reporte <> "" Then
            Dim wbusqueda As String = UCase(txtfiltrocliente.Text)
            Dim myCurrencyManager As CurrencyManager
            myCurrencyManager = CType(Me.BindingContext(dtvcliente), CurrencyManager)
            Dim INTnewpos As Integer
            dtvcliente.RowFilter = STRorden_det_reporte & " like '%" & txtfiltrocliente.Text & "%'"
            myCurrencyManager.Position = INTnewpos
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        If MessageBox.Show("Seguro de actualizar el Tipo de Cambio de la Planilla?" & vbCrLf & "¿Desea continuar?", "Sistemas", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            Try
                ...
                If planilladetBL.Actualizar_Tipo_Cambio_Planilla_Cobro_PF(txt_numero_planilla.Text.Trim, CDbl(txt_tc_planilla.Text.Trim)) Then
                    planilladetBL = Nothing
    ...
    Private Sub Sumatoria_Planilla_Referencia()
        Try
            ...
            dt_detalle_sumatoria = planillaBL.get_planilla_Sumatoria_Ref_Pago_Cliente_PF(num_planilla)
   ...
   ByVal e As DataGridViewEditingControlShowingEventArgs) Handles dgv_pendientes.EditingControlShowing
    ...
    Private Sub RadioButton1_CheckedChanged(sender As Object, e As EventArgs) Handles RadioButton1.CheckedChanged
        If RadioButton1.Checked = True Then
            ...
            btn_buscar_fp.Select()
        End If
    End Sub

    Private Sub btn_buscar_fp_Click(sender As Object, e As EventArgs) Handles btn_buscar_fp.Click
        Cargar_Documentos_Pendientes_por_fechas(dtpinicio_fecha_pago.Value.ToString("dd/MM/yyyy"), dtpfecha_fin_pago.Value.ToString("dd/MM/yyyy"))
        dgv_pendientes.Focus()
    End Sub
End Class