...
Public Class frm_CX_LT_PlanillaPagos
    ...
    Dim STRorden_detalle As String = String.Empty
    ...
    Dim clsClienteBl As ClsOperaciones.RECEIVABLE_LIST_PAY
    ...
    Dim _serie_anticipos As String = String.Empty
    Dim _numero_anticipos As String = String.Empty
    Public GRUPO_PAGOS As String


    Private Sub CONTROLES(ByVal opcion As Boolean, ByVal _indice As Integer)
        tc_planilla.TabPages(0).Enabled = False
        tc_planilla.TabPages(1).Enabled = False
        tc_planilla.TabPages(2).Enabled = False
    ...
    Private Sub Ayuda_Clientes(Optional ByVal _CodigoRuc As String = "")
        Try
            ...
            If _CodigoRuc = String.Empty Then
                ...
                frm.Close()
            Else
                clsClienteBl = New ClsOperaciones.RECEIVABLE_LIST_PAY
                dtDetalleTerceros = New DataTable("Clientes_RUC")
                dtDetalleTerceros = clsClienteBl.ManualClienteRuc_Pago_Proveedor(_CodigoRuc)
    ...
    Private Sub Ayuda_Pagos_Fecha(ByVal _fecha_inicio As String, ByVal _fecha_fin As String)
        Try
            Me.Cursor = Cursors.WaitCursor
            If _fecha_inicio <> "" And _fecha_fin <> "" Then
                Dim saldosBL As New ClsOperaciones.CUSTOMER_BALANCE
                dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Pago_Proveedores_x_fecha(_fecha_inicio, _fecha_fin)
                ...
                Next
                dgv_pendientes.Columns("CUSTOMER_ID").Visible = True
                dgv_pendientes.Columns("TERCERO").Visible = True
                dgv_pendientes.Columns("CUSTOMER_ID").Width = 100
                dgv_pendientes.Columns("TERCERO").Width = 280
    ...
    Private Sub Ayuda_Comprobantes_(Optional ByVal _Codigo As String = "")
        Try
            'Opcion  C AS BUSQUEDA POR CODIGO, R BUSQUEDA POR RUC ..
            Dim frm As New frmBuscar
            If _Codigo = String.Empty Then
                Dim sql As String = String.Empty
                sql = "CX_SP_S_COMPROBANTE"
                frm.CadenaConsulta = sql
                frm.Titulo = "Comprobantes"
                frm.ShowDialog()
                If frm.Data_Matriz.Rows.Count > 0 Then
                    txt_cod_ane.Text = frm.Data_Matriz.Rows(0).Item(0).ToString 'frm.Data_Matriz.Rows(0).Item(0).ToString
                    txt_des_ane.Text = frm.Data_Matriz.Rows(0).Item(1).ToString
            ...
            Else
                clsClienteBl = New ClsOperaciones.RECEIVABLE_LIST_PAY
                dtDetalleTerceros = New DataTable("Comprobantes")
                dtDetalleTerceros = clsClienteBl.ManualComprobantes(_Codigo, CDate(txt_des_ane.Text).ToShortDateString)
                If dtDetalleTerceros.Rows.Count() <> 0 Then
                    'txt_cod_ane.Text = dtDetalleTerceros.Rows(0).Item(2).ToString
                    'txt_des_ane.Text = dtDetalleTerceros.Rows(0).Item(1).ToString
                    If txt_cod_ane.Text <> String.Empty Then
...
ByVal e As System.Windows.Forms.KeyPressEventArgs)
   ...
   ByVal e As DataGridViewEditingControlShowingEventArgs) Handles dgv_pendientes.EditingControlShowing
    ...
    Private Sub frm_CX_LT_PlanillaCob_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        WindowState = FormWindowState.Maximized
        Call Cargar_Tipo_Cobranza()
        'Call Cargar_Bancos()
...
#Region "subprocesos"
    ...
    Private Sub Cargar_Documentos()
        ...
        cmb_tipo_doc.DataSource = documentosBL.get_DocumentosNC_Pagos()
    ...
    Private Sub Cargar_Tipo_Cobranza()
        ...
        cmb_tipo_cob.DataSource = tipoCobranzaBL.get_Tipos_PagosProveedor(GRUPO_PAGOS)
    ...
    Private Sub Cargar_Documentos_Pendientes()
        ...
        _fecha = ModFunciones.Ultimo_Dia_Mes(CDate(txt_fecha_planilla.Text)).ToString("dd/MM/yyyy")
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes_Pago_Proveedores(rb_Proveedor.Checked, txt_cod_ane.Text, txt_des_ane.Text, _fecha)
        dgv_pendientes.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'dgv_pendientes.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        saldosBL = Nothing
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
            dgv_pendientes.Rows(i).Cells("COL_SALDO").Value = dgv_pendientes.Rows(i).Cells("COL_MONTO_SALDO").Value
        Next
        dgv_pendientes.Columns("CUSTOMER_ID").Visible = False
        dgv_pendientes.Columns("TERCERO").Visible = False
        dgv_pendientes.Update()
    End Sub

    Private Sub Cargar_Documentos_Pendientes_Tipo_Documento()
        Dim saldosBL As New ClsOperaciones.CUSTOMER_BALANCE
        Dim _fecha As String
        _fecha = ModFunciones.Ultimo_Dia_Mes(CDate(txt_fecha_planilla.Text)).ToString("dd/MM/yyyy")
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes_Pago_Proveedores_Tipo_Documento(rb_tipo_documento.Checked, txt_cod_ane.Text, txt_des_ane.Text, _fecha, txt_tipo_documento.Text)
        dgv_pendientes.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'dgv_pendientes.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        saldosBL = Nothing
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
    ...
    Private Sub Cargar_Documentos_Pendientes_x_fecha()
        Dim saldosBL As New ClsOperaciones.CUSTOMER_BALANCE
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes_Pago_Proveedores(rb_Proveedor.Checked, txt_cod_ane.Text, txt_des_ane.Text, txt_fecha_planilla.Text)
        dgv_pendientes.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        'dgv_pendientes.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
        saldosBL = Nothing
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
    ...
    Private Sub Consultar_Planillas_Cab_xfecha()
        Dim planillaBL As New ClsOperaciones.RECEIVABLE_LIST_PAY
        dtvC = New DataView
        ...
        dtvC = planillaBL.get_planilla_x_fecha_Pago_Proveedor(fecha_ini, fecha_fin, GRUPO_PAGOS).DefaultView
        dgv_planillas.CommitEdit(DataGridViewDataErrorContexts.Commit)
        dgv_planillas.DataSource = dtvC
        ' dgv_planillas.Columns(0).SortMode = DataGridViewColumnSortMode.NotSortable
        dgv_planillas.Columns(0).HeaderText = "N° Planilla"
        dgv_planillas.Columns(1).HeaderText = "Fecha"
        dgv_planillas.Columns(2).HeaderText = "Total Pagos Soles"
        dgv_planillas.Columns(3).HeaderText = "Total Pagos Dolares"
...
#End Region
    ...
    Private Sub Tool_Salir_Click(sender As Object, e As EventArgs)
        Me.Close()
    End Sub
    ...
    Private Function Verificar_Apertura_Cierre_Modulos(ByVal Mes As String, ByVal anyo As String) As Boolean
        ...
        Try
            ...
            If dtCierre.Rows.Count() > 0 Then
                MsgBox("El mes seleccionado se encuentra CERRADO," & Chr(13) & "APERTURAR para generar la Planilla de Pagos.", MsgBoxStyle.Critical)
                Estado = False
                Exit Try
            End If
    ...
    Private Sub btn_crear_planilla_Click(sender As Object, e As EventArgs) Handles btn_crear_planilla.Click
        ...
        dtTipoCamb_tc = cl.Verifircar_Planilla_Pagos(dtp_fec_nueva_pla.Value.ToString("dd/MM/yyyy"), GRUPO_PAGOS)
        If dtTipoCamb_tc.Rows.Count() > 0 Then
            MsgBox("Existe una planilla con la fecha elegida, proceda a modificar dicha planilla.", MsgBoxStyle.Exclamation, "Sistema")
            Exit Sub
        End If
        ...
        With planillaBE
            ...
            Else
                MsgBox("No existe el Tipo de Cambio de la fecha elegida.", MsgBoxStyle.Exclamation, "Sistema")
                Exit Sub
            End If
            ...
            .GRUPO_COBRANZA = GRUPO_PAGOS
        End With
        planillaBL.Insert_Pago_Proveedor(planillaBE, True, numero_planilla)
        planillaBE = Nothing
        planillaBL = Nothing
    ...
    Private Sub btn_busca_cli_Click(sender As Object, e As EventArgs)
    ...
    Private Sub btn_grabar_det_Click(sender As Object, e As EventArgs) Handles btn_grabar_det.Click
        ...
        Dim _numero_compra As String = ""
        Dim _proveedor As String = ""
        Dim _tipo_documento As String = ""
        Dim _serie_documento As String = ""
        Dim _numero_documento As String = ""
        Dim _fecha_pago As String = ""
        Dim _monto_pago As Double = 0.0
        ...
        If estado = False Then
            MessageBox.Show("Debe seleccionar al menos un item.", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            dgv_pendientes.Focus()
            Exit Sub
        End If
            ...
            For Each row As DataGridViewRow In dgv_pendientes.Rows
                If row.Cells(0).Value = True Then
                    If row.Cells(1).Value = 0 Then
                        MessageBox.Show("Al seleccionar un registro debe ingresar una cantidad diferente de 0.", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        Exit Sub
                    End If
                End If
            Next
        ...
        If cmb_tipo_cob.SelectedIndex = -1 Then
            MessageBox.Show("Seleccione tipo de pago", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            cmb_tipo_cob.Focus()
            Exit Sub
        End If

        If chk_comprobante_retencion.Checked Then
            ...
            For i As Integer = 0 To dgv_pendientes.RowCount - 1
                If dgv_pendientes.Rows(i).Cells("chk_Sel").Value Then
                    ...
                    dtCabeceraoC = planillaBL.Get_Datos_Verifica_Retencion(dgv_pendientes.Rows(i).Cells("COL_NUMBER_REF").Value.ToString)
                    ...
                    dtContribuyente = planillaBL.Get_ValidaRetencion(dgv_pendientes.Rows(i).Cells("CUSTOMER_ID").Value)
                    ...
                    'Dim dtCabeceraoC As New DataTable
                    dtCabeceraoC = planillaBL.get_Verifica_Ubigeo_Proveedor(dgv_pendientes.Rows(i).Cells("COL_NUMBER_REF").Value.ToString)
                    If dtCabeceraoC.Rows.Count() <> 0 Then
                        MsgBox("Actualize el ubigeo del Proveedor!!!", MsgBoxStyle.Exclamation)
                        Exit Sub
                    End If

                    If CDbl(dgv_pendientes.Rows(i).Cells("MONTO_ORIGINAL_SOLES").Value) < 700 Then
                        ...
                        Exit Sub
                    End If
                    _numero_compra = dgv_pendientes.Rows(i).Cells("COL_NUMBER_REF").Value.ToString
                    Exit For
                End If
        ...
        If gb_banco.Enabled Then
            ...
            If txt_num_ref_ban.Text.Trim.Length = 0 Then
                MessageBox.Show("ingrese un numero de referencia", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Stop)
        ...
        If gb_aplidoc.Enabled Then
            ...
            Dim planillaBL As ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
            planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
            Dim dt_detalle As DataTable
            dt_detalle = New DataTable
            dt_detalle = planillaBL.VerificandoNumeracionDocumentosNC_Pago_Proveedor(txt_cod_ane.Text, txtserieDoc.Text & txtnumDoc.Text)
        ...
        If gb_aplicacionSaldos.Enabled = True Then
            ...
            If txtnumerosaldo.Text = String.Empty Then
                MsgBox("Debe ingresar un  numero de documento.", MsgBoxStyle.Exclamation)
        ...
        Me.Cursor = Cursors.WaitCursor
        Dim planilladetBL As ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
        planilladetBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
        Dim planilladetBE As ClsEntidades.RECEIVABLE_LIST_LINE_PAY
        Dim ls_detalle As New List(Of ClsEntidades.RECEIVABLE_LIST_LINE_PAY)
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
            planilladetBE = New ClsEntidades.RECEIVABLE_LIST_LINE_PAY
            If dgv_pendientes.Rows(i).Cells("chk_Sel").Value Then
                If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
                    If cmb_tipo_doc.SelectedValue = "NC" Or cmb_tipo_doc.SelectedValue = "NCE" Then
                        If dgv_pendientes.Rows(i).Cells("COL_DOCUMENT_ID").Value = "NC" Or dgv_pendientes.Rows(i).Cells("COL_DOCUMENT_ID").Value = "NCE" Then
                            MessageBox.Show("Debe seleccionar un TipoDoc diferente a Nota Credito/Nota de Credito Especial", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                            Me.Cursor = Cursors.Default
                            Exit Sub
                        End If
                    End If
                ...
                With planilladetBE
                    ...
                    _fecha_pago = .DATE_PLA
                    If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
                        .AMOUNT = (Math.Abs(CDbl(dgv_pendientes.Rows(i).Cells("COL_SALDO").Value.ToString)))
                    Else
                        .AMOUNT = Math.Abs(CDbl(dgv_pendientes.Rows(i).Cells("COL_SALDO").Value.ToString))
                    End If
                    ...
                    If txt_gastos_financieros.Text = "" Then
                        .AMOUNT_INTERES = 0
                    Else
                        .AMOUNT_INTERES = CDbl(txt_gastos_financieros.Text)
                    End If

                    If txt_gastos_bancarios.Text = "" Then
                        .AMOUNT_GASTOS_BANCARIOS = 0
                    Else
                        .AMOUNT_GASTOS_BANCARIOS = CDbl(txt_gastos_bancarios.Text)
                    End If
                    ...
                    End If
                    .SELL_RATE = _tipo_cambio_planilla 'dgv_pendientes.Rows(i).Cells("COL_TC").Value.ToString
                    .CREATE_DATE = Now.Date.ToShortDateString
                    .USER_ID = LibComunVar.ClsVarComun.USUARIO
                    ...
                    If gb_banco.Enabled Then
                        ...
                        .DOC_REF = TraerCodigo(txtdocRefBancos.Text).Trim
                    ...
                    If gb_Efectivo.Enabled = True And gb_aplidoc.Enabled = False And gb_banco.Enabled = False Then
                        .DOC_REF = TraerCodigo(txt_efectivo_doc_Ref.Text).Trim
                    ...
                    If gb_aplicacionSaldos.Enabled = True And gb_Efectivo.Enabled = False And gb_aplidoc.Enabled = False And gb_banco.Enabled = False Then
                        .DOC_REF = TraerCodigo(txtreferenciaSaldo.Text).Trim
                        If _serie_anticipos = "" And _numero_anticipos = "" Then
                            .SERIE_REF = ""
                            .NUM_REF = txtnumerosaldo.Text.Trim
                        Else
                            .SERIE_REF = _serie_anticipos
                            .NUM_REF = _numero_anticipos
                        End If
                    End If
                    .LIST_ID = 0
                    .IS_CASH_BANK = "0"
                    ...
                    _serie_documento = .NUMBER_SERIE
                    .NUMBER_DOCUMENT = dgv_pendientes.Rows(i).Cells("NUM_DOC_AUX").Value.ToString
                    _numero_documento = .NUMBER_DOCUMENT
                    .MEDIOS_PAGO = 0
                    If Trim(.DOC_REF) = "CD" Then
                        .DATE_REF = dtp_detrac.Value.ToString("dd/MM/yyyy")
                    Else
                        .DATE_REF = ""
                    End If
                    If txt_ing_redondeo.Text = "" Then
                        .INGRESOS_POR_REDONDEO = CDbl(0)
                    Else
                        .INGRESOS_POR_REDONDEO = CDbl(txt_ing_redondeo.Text)
                    End If

                    If txt_gastos_redondeo.Text = "" Then
                        .GASTOS_POR_REDONDEO = CDbl(0)
                    Else
                        .GASTOS_POR_REDONDEO = CDbl(txt_gastos_redondeo.Text)
                    End If
            ...
            End If
        Next
        Dim estado_de_grabado As Boolean = False
        If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
            If cmb_tipo_doc.SelectedValue = "NC" Or cmb_tipo_doc.SelectedValue = "NCE" Then
                estado_de_grabado = planilladetBL.InsertNC_Pago_Proveedor(ls_detalle, cmb_tipo_doc.SelectedValue.ToString, txtserieDoc.Text, txtnumDoc.Text, _
                                                      (Math.Abs(CDbl(txtmonto.Text))) * -1, txt_cod_mon.Text, FechaActualPlanilla)
            Else
                estado_de_grabado = planilladetBL.Insert_Pago_Proveedor(ls_detalle, txt_cod_mon.Text, FechaActualPlanilla)
            End If
        Else
            estado_de_grabado = planilladetBL.Insert_Pago_Proveedor(ls_detalle, txt_cod_mon.Text, FechaActualPlanilla)
        End If
        ...
        If estado_de_grabado = True Then

            If chk_comprobante_retencion.Checked Then
                Genera_CR(0, _monto_pago, _proveedor, _tipo_documento, _serie_documento, _numero_documento, dtp_fecha_cr.Value.ToString("dd/MM/yyyy"))
            End If

            MessageBox.Show("Proceso efectuado correctamente!", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Information)
            cmb_tipo_cob.SelectedIndex = -1
            Blank()
    ...
    Private Sub Genera_CR(ByVal _id As Integer, ByVal _monto As Double, ByVal _proveedor As String, ByVal _tipo_documento As String, ByVal _serie_documento As String,
                          ByVal _numero_documento As String, ByVal _fecha_pago As String)
        Try
            Me.Cursor = Cursors.WaitCursor
            ...
            If MessageBox.Show("¿Desea generar el Comprobante de Retención?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Dim planilladetBL As ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                planilladetBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                If planilladetBL.Genera_Comp_Retencion_Pagos(_proveedor, _tipo_documento, _serie_documento, _numero_documento, _fecha_pago, _id, _monto, txt_cod_mon.Text) Then
                    If Generar_XML_Comprobante_retencion(_id, STRmontotexto, nombre_xml, nombre_pdf) = False Then
                        MsgBox("El documento " & nombre_pdf & " no genero el archivo XML." & Chr(13) & "Realizar la revisión.", MsgBoxStyle.Critical, "Sistemas")
                    Else
                        ...
                        Else
                            ...
                            Dim msj As String = ""
                            msj = "Documento Generado Correctamente" & vbCrLf & "Nro Documento: " & nombre_pdf & vbCrLf & "Desea imprimir el Comprobante de Retención?"
    ...
    Private Function Generar_XML_Comprobante_retencion(ByVal Numero_compra As Integer, ByVal _monto_texto As String, ByRef _nombre_xml As String, ByRef _nombre_pdf As String) As Boolean
        Try
            clsFacturaBl = New ClsNegocio.RECEIVABLE
            If clsFacturaBl.Generar_Xml_Comprobante_Retencion_Pagos(Numero_compra, Ruta_FE() & "\InputXML", _monto_texto, _nombre_xml, _nombre_pdf) Then
                Return True
            Else
                Return False
            End If
    ...
    Private Sub Blank()
        ...
        txt_itf.Text = String.Empty
        txt_gastos_financieros.Text = String.Empty
        txt_gastos_bancarios.Text = String.Empty
        txt_itf.Text = String.Empty
        cmb_banco.SelectedIndex = -1
        cmb_cta_cte.SelectedIndex = -1
        txtserieDoc.Text = String.Empty
        ...
        _serie_anticipos = ""
        _numero_anticipos = ""
        txt_ing_redondeo.Text = 0
        txt_gastos_redondeo.Text = 0
        chk_comprobante_retencion.Checked = False
        lbl_fecha_cr.Visible = False
        dtp_fecha_cr.Visible = False
    ...
    Private Sub cmb_tipo_cob_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_tipo_cob.SelectedIndexChanged
        ...
        If bol_activar_cmbs Then
            If cmb_tipo_cob.SelectedIndex <> -1 Then
                ...
                tipocobBL.get_Tipo_x_Cod_Pago(tipocobBE)
                tipocobBL = Nothing
                txtserieDoc.Text = String.Empty
                txtnumDoc.Text = String.Empty
                txtmonto.Text = "0.0"
                ...
                If tipocobBE.BANK_ID = 1 Then 'pagos con banco
                    ...
                    dtSolucion = tipocobBL.Conceptos_Cobranza_Pago(cmb_tipo_cob.SelectedValue.ToString)
                    ...
                    cmb_banco.Focus()
                End If
                'If tipocobBE.IS_APPL = 1 And tipocobBE.TRANS_TYPE = 0 Then 'pagos con efectivo
                If tipocobBE.TRANS_TYPE = 0 Then 'pagos con efectivo
                    gb_banco.Enabled = False
                    gb_aplidoc.Enabled = False
                ...
                If tipocobBE.IS_APPL = 1 And tipocobBE.TRANS_TYPE = 3 Then 'pago con aplicacion de NC
                ...
                If tipocobBE.IS_APPL = 0 And tipocobBE.TRANS_TYPE = 7 Then 'pago con aplicacion de SALDO
                    gb_banco.Enabled = False
                    gb_aplidoc.Enabled = False
                    gb_Efectivo.Enabled = False
            ...
            End If
        End If
        Calculando_Montos(txt_cod_mon.Text, tipocobBE.RECEIVE_ID, _tipo_cambio_planilla)
    End Sub



    Private Sub Calculando_Montos(ByVal MonedaCobranza As String, ByVal Concepto_Pago As String, ByVal T_cambio As Double)
        ...
        Try
            ...
            For i As Integer = 0 To dgv_pendientes.RowCount - 1
                If dgv_pendientes.Rows(i).Cells("chk_Sel").Value = False Then
                    Continue For
                Else
                    AMOUNT = dgv_pendientes.Rows(i).Cells("COL_SALDO").Value.ToString ' dgv_pendientes.Rows(i).Cells("COL_MONTO_SALDO").Value.ToString 'dgv_pendientes.Rows(i).Cells("COL_SALDO").Value.ToString
                    AMOUNT_SALDO = dgv_pendientes.Rows(i).Cells("COL_MONTO_SALDO").Value.ToString


                    CURRENCY_ID = dgv_pendientes.Rows(i).Cells("COL_COD_MON").Value.ToString
                    If dgv_pendientes.Rows(i).Cells("COL_DOCUMENT_ID").Value <> "NC" And dgv_pendientes.Rows(i).Cells("COL_DOCUMENT_ID").Value <> "NCE" Then
                        If CURRENCY_ID = "ME" Then
                            If MonedaCobranza = "ME" Then
                                AMOUNT = AMOUNT
                            Else
                                If Concepto_Pago = "D1" Or Concepto_Pago = "D2" Then
                                    AMOUNT = AMOUNT * T_cambio
                                Else
                                    AMOUNT = AMOUNT * T_cambio ' dgv_pendientes.Rows(i).Cells("COL_TC").Value
                                End If
                        ...
                        ElseIf CURRENCY_ID = "MN" Then
                            If MonedaCobranza = "MN" Then
                                AMOUNT = AMOUNT
                            Else
                                'AMOUNT = AMOUNT / dgv_pendientes.Rows(i).Cells("COL_TC").Value
                    ...
                    Else
                        If CURRENCY_ID = "ME" Then
                            ...
                            Else
                                'AMOUNT = (Math.Abs(AMOUNT * dgv_pendientes.Rows(i).Cells("COL_TC").Value) * -1)

                                If Concepto_Pago = "D1" Or Concepto_Pago = "D2" Then
                                    AMOUNT = (Math.Abs(AMOUNT * T_cambio) * -1)
                                Else
                                    AMOUNT = (Math.Abs(AMOUNT * T_cambio) * -1) 'dgv_pendientes.Rows(i).Cells("COL_TC").Value) * -1)
                                End If
                        ...
                        End If
                    End If
                    dgv_pendientes.Rows(i).Cells("COL_SALDO").Value = IIf(String.IsNullOrEmpty(AMOUNT), 0, AMOUNT)
                    dgv_pendientes.Update()
                End If
            Next
        Catch ex As Exception
    ...
    Private Sub cmb_banco_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_banco.SelectedIndexChanged
        ...
        If bol_activar_cmbs Then
            Dim ctasctesbl As New ClsOperaciones.ACCOUNT_BANK_PAY
    ...
    Private Sub dgv_pendientes_EditModeChanged(sender As Object, e As EventArgs) Handles dgv_pendientes.EditModeChanged
    ...
    Private Sub txt_cod_ane_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_cod_ane.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            Else
                Ayuda_Comprobantes_(txt_cod_ane.Text)
            End If
        End If
    End Sub
    ...
    Private Sub btn_edit_pla_Click(sender As Object, e As EventArgs) Handles btn_edit_pla.Click
        If dgv_planillas.Rows.Count() = 0 Then
            ...
            Exit Sub
        End If
        Call Editar_Planilla()
        Call Sumar_Montos_DetPlanilla()
        CONTROLES(True, 1)
    ...
    Private Sub Edicion_Planilla()
        ...
        dt_detalle = New DataTable
        dtvD = New DataView
        dt_detalle = planillaBL.get_planilla_Det_xID_Pago_Proveedor(num_planilla)
        If dt_detalle.Rows.Count() <> 0 Then
            ...
            dgv_detalles.Columns(0).Width = 60
            dgv_detalles.Columns(0).HeaderText = "NroPlanilla"
            ...
            dgv_detalles.Columns(5).Width = 60
            dgv_detalles.Columns(5).HeaderText = "Codigo"
            dgv_detalles.Columns(6).Width = 100
            dgv_detalles.Columns(6).HeaderText = "Proveedor"
            dgv_detalles.Columns(7).Width = 50
            dgv_detalles.Columns(7).HeaderText = "Codigo"
            dgv_detalles.Columns(8).Width = 100
            dgv_detalles.Columns(8).HeaderText = "Vendedor"
            ...
            dgv_detalles.Columns(10).DefaultCellStyle.Format = "##,##0.000"
            dgv_detalles.Columns(10).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            dgv_detalles.Columns(11).Width = 40
            dgv_detalles.Columns(11).HeaderText = "TipDocR"
            dgv_detalles.Columns(12).Width = 80
            dgv_detalles.Columns(12).HeaderText = "NroDocRef"
            dgv_detalles.Columns(13).Width = 40
            dgv_detalles.Columns(13).HeaderText = "Codigo"
            dgv_detalles.Columns(14).Width = 40
    ...
    Private Sub Editar_Planilla()
        ...
        _tipo_cambio_planilla = CDbl(dgv_planillas.CurrentRow.Cells("SELL_RATE").Value.ToString)
        FechaActualPlanilla = dgv_planillas.CurrentRow.Cells("FECHA").Value.ToString
        ...
        txt_tc_planilla.Text = _tipo_cambio_planilla
        ...
        If dt_detalle.Rows.Count() <> 0 Then
            ...
            dgv_detalles.Columns(5).HeaderText = "Ruc"
            dgv_detalles.Columns(6).HeaderText = "Proveedor"
            dgv_detalles.Columns(9).HeaderText = "Mon"
            dgv_detalles.Columns(10).HeaderText = "Monto"
            ...
            dgv_detalles.Columns(15).HeaderText = "Nro Cuenta"
            dgv_detalles.Columns(15).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            dgv_detalles.Columns(16).Visible = False
            dgv_detalles.Columns(19).Visible = False
            dgv_detalles.Columns(20).HeaderText = "Concepto de Pago"
            ...
            dgv_detalles.Columns(23).HeaderText = "Núm. Referencia"
            dgv_detalles.Columns(23).DefaultCellStyle.Format = "##,##0.000"
            dgv_detalles.Columns(23).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            dgv_detalles.Columns(24).HeaderText = "Interes Moratorio"
            dgv_detalles.Columns(24).DefaultCellStyle.Format = "##,##0.000"
            dgv_detalles.Columns(24).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            dgv_detalles.Columns(25).HeaderText = "I.T.F."
            dgv_detalles.Columns(25).DefaultCellStyle.Format = "##,##0.000"
            dgv_detalles.Columns(25).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            dgv_detalles.Columns(26).HeaderText = "Gastos Bancarios"
            dgv_detalles.Columns(26).DefaultCellStyle.Format = "##,##0.000"
            dgv_detalles.Columns(26).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            dgv_detalles.Columns(27).HeaderText = "Observaciones"

            dgv_detalles.Columns(28).HeaderText = "Tipo de Cambio"
            dgv_detalles.Columns(28).DefaultCellStyle.Format = "##,##0.000"

            dgv_detalles.Columns(29).HeaderText = "Ingresos por Redondeo"
            dgv_detalles.Columns(29).DefaultCellStyle.Format = "##,##0.00"

            dgv_detalles.Columns(30).HeaderText = "Gastos por Redondeo"
            dgv_detalles.Columns(30).DefaultCellStyle.Format = "##,##0.00"
            ...
            STRorden_detalle = dgv_detalles.Columns(0).Name & "+" & dgv_detalles.Columns(3).Name & "+" & dgv_detalles.Columns(4).Name & "+" & dgv_detalles.Columns(5).Name
            lblCantidadDetall.Text = "Se encontraron " & dtvD.Count & " registros"
        Else
            dgv_detalles.DataSource = Nothing
        End If
    ...
    Private Sub Sumar_Montos_DetPlanilla()
        ...
        For i As Integer = 0 To dgv_detalles.Rows.Count - 1
            If dgv_detalles.Rows(i).Cells("CURRENCY_ID").Value = "MN" Then
                s += Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT").Value) + Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT_INTERES").Value) + Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT_GASTOS_BANCARIOS").Value)
            Else
                d += Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT").Value) + Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT_INTERES").Value) + Math.Abs(dgv_detalles.Rows(i).Cells("AMOUNT_GASTOS_BANCARIOS").Value)
            End If
        Next
        mtb_tot_soles.Text = Format(Double.Parse(s), "##,##0.00")
        mtb_tot_dolares.Text = Format(Double.Parse(d), "##,##0.00")
    ...
    Private Sub btnEliminarRegistro_Click(sender As Object, e As EventArgs) Handles btnEliminarRegistro.Click
        ...
        If MessageBox.Show("Se va a eliminar el registro" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            Dim TipoCambio_detalle As Double = 0
            ...
            If dtValidacion.Rows.Count > 0 Then
                _genera_validacion_al_eliminar = dtValidacion.Rows(0).Item("IS_DELETE_PLANILLA").ToString
            End If
            ...
            Else
                ...
                Item = dgv_detalles.CurrentRow.Cells("ITEM").Value
                ...
                TipoDocRef = dgv_detalles.CurrentRow.Cells("DOCUMET_ID").Value
                NumDocRef = dgv_detalles.CurrentRow.Cells("NUMBER_DOCUMENT").Value
                TipoDocCobranza = dgv_detalles.CurrentRow.Cells("DOC_REF").Value
                NumDocCobranza = dgv_detalles.CurrentRow.Cells("NUM_REF").Value
                TipoCambio_detalle = dgv_detalles.CurrentRow.Cells("SELL_RATE").Value
                ...
                dt_detalle = New DataTable("DatosDocumento_Pago")
                Consulta = "SELECT * FROM CUSTOMER_BALANCE_PAY  WHERE CUSTOMER_ID= '" & CodCliente & "' AND DOCUMENT_ID='" & TipoDocRef & "' AND NUMBER_DOC='" & numeroDoc & "'"
                ...
                If dt_detalle.Rows.Count() <> 0 Then
                    ...
                    If TipoD <> "NC" And TipoD <> "NCE" Then
                        If Moneda = "ME" Then
                            If MonDoc = "ME" Then
                                MontodoC = MontodoC
                            Else
                                MontodoC = MontodoC / TipoCambio_detalle
                            End If
                    ...
                    Else
                        If Moneda = "ME" Then
                            ...
                            Else
                                MontodoC = (Math.Abs(MontodoC / TipoCambio_detalle)) * -1
                            End If
                        ElseIf Moneda = "MN" Then
                            If MonDoc = "MN" Then
                                MontodoC = (Math.Abs(MontodoC)) * -1
                    ...
                    End If
                    planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                    If planillaBL.ActualizacionDocumentosEliminados_Pago_Proveedor(CodCliente, TipoDocRef, numeroDoc, MontodoC) = True Then
                        If TipoDocCobranza = "NC" Or TipoDocCobranza = "NCE" Then
                            Dim MonedaNC As String = String.Empty
                            Consulta = String.Empty
                            dt_detalle = New DataTable("DatosNC_Pago")
                            Consulta = "SELECT * FROM CUSTOMER_BALANCE_PAY  WHERE CUSTOMER_ID= '" & CodCliente & "' AND DOCUMENT_ID='" & TipoDocCobranza & "' AND NUMBER_DOC='" & NumDocCobranza & "'"
                            dt_detalle = clsBuscar.MostrandoDescripcion(Consulta)
                            If dt_detalle.Rows.Count() <> 0 Then
                                ...
                                End If
                                planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                                If planillaBL.ActualizacionDocumentosEliminados_Pago_Proveedor(CodCliente, TipoDocCobranza, NumDocCobranza, (Math.Abs(CDbl(MontodoC))) * -1) = False Then
                                    MsgBox("El saldo de la NC no pudo restaurarse. Verifique.!!!", MsgBoxStyle.Exclamation)
                                End If
                            End If
                        End If
                        planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                        If planillaBL.EliminarDetallesPlanillas_Pago_Proveedor(txt_numero_planilla.Text, Item, tipoDoc, numeroDoc, CodCliente) = True Then
                            MsgBox("Eliminado Correctamente", MsgBoxStyle.Information)
                        Else
                            MsgBox("Error de eliminacion del detalle. Verifique!", MsgBoxStyle.Critical)
                        End If
                    Else
                        MsgBox("Error de eliminacion", MsgBoxStyle.Critical)
                    End If
                Else
                    MsgBox("El registro no se puede eliminar,el documento no existe en la cartera de pagos. Verifique.!!!", MsgBoxStyle.Exclamation)
                End If
                Editar_Planilla()
            End If
    ...
    Private Function VerficarNotaC() As Boolean
        Try
            txtnumDoc.Text = String.Format("{0:0000000}", CInt(txtnumDoc.Text.Trim))
            txtnumDoc.Text = txtserieDoc.Text & txtnumDoc.Text
            Dim planillaBL As ClsOperaciones.RECEIVABLE_LIST_LINE
            planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE
            Dim dt_detalle As DataTable
            dt_detalle = New DataTable
            dt_detalle = planillaBL.VerificandoNumeracionDocumentosNC(txt_cod_ane.Text, txtnumDoc.Text)
            If dt_detalle.Rows.Count() <> 0 Then
    ...
    Private Sub txtbuscar_TextChanged(sender As Object, e As EventArgs) Handles txtbuscar.TextChanged
        If STRorden_detalle <> "" Then
            Dim wbusqueda As String = UCase(txtFiltro.Text)
            Dim myCurrencyManager As CurrencyManager
            myCurrencyManager = CType(Me.BindingContext(dtvD), CurrencyManager)
            Dim INTnewpos As Integer
            dtvD.RowFilter = STRorden_detalle & " like '%" & txtbuscar.Text & "%'"
            myCurrencyManager.Position = INTnewpos
    ...
    Private Sub ImprimirVoucher(ByRef _DOCUMENT_ID As String)
        Try
            ...
            If MessageBox.Show("¿Desea imprimir el Documento ?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                Dim SQL As String = String.Empty
                SQL = "SELECT *  FROM DBO.CO_TB_ASIENTO_CAB WHERE AC_IDSUBDIARIO = '04'   AND AC_ID_PAGO ='" & _DOCUMENT_ID & "'"
    ...
    Private Sub btnContabilizar_Click(sender As Object, e As EventArgs) Handles btnContabilizar.Click
        Try
            ...
            _FECHA_PLANILLA = dgv_planillas.Item(1, dgv_planillas.CurrentRow.Index).Value
            ...
            If MessageBox.Show("¿Desea numeracion Automatica para el Comprobante ?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                ...
                _NUMERACION = ac.get_Ult_num_voucher("04", fecha.Year, Format(fecha.Month, "00"))
                ac = Nothing
            Else
                ...
                dtcodigo = ClsasientoBl.ValidarCodigo("04", codigo, CDate(_FECHA_PLANILLA).Year, CDate(_FECHA_PLANILLA).Month)
            ...
            End If
            Me.Cursor = Cursors.WaitCursor
            Dim ClsCobranzaBl As New ClsOperaciones.RECEIVABLE_LIST_PAY
            If ClsCobranzaBl.Contabilizar_Caja_Pagos_Proveedores(_CODIGO_PLANILLA, CDate(_FECHA_PLANILLA), _NUMERACION, ProgressBar1) = True Then
    ...
    Private Sub cmb_tipo_cob_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_glosa.KeyPress, txt_cta_conta.KeyPress, txt_cod_mon.KeyPress, mtb_fec_cob.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Then
            ...
            Else
                Try
                    ...
                    dt_detalle = New DataTable
                    dt_detalle = planillaBL.VerificandoBancodeCuentaPago(txt_cta_conta.Text)
                    If dt_detalle.Rows.Count() <> 0 Then
    ...
    Private Sub txtnumDoc_Leave(sender As Object, e As EventArgs) Handles txtnumDoc.Leave
        If txtnumDoc.Text = String.Empty Then Exit Sub
        If cmb_tipo_doc.SelectedValue = "NC" Or cmb_tipo_doc.SelectedValue = "NCE" Then
            If dgv_pendientes.CurrentRow Is Nothing Then Exit Sub
            For Each row As DataGridViewRow In dgv_pendientes.Rows
                If row.Cells(0).Value = True Then
                    If row.Cells("COL_DOCUMENT_ID").Value = "NC" Or row.Cells("COL_DOCUMENT_ID").Value = "NCE" Then
                        MessageBox.Show("Debe marcar con un check un TipoDoc diferente a Nota Credito", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                        Exit Sub
                    End If
                End If
    ...
    Private Sub btn_elimianr_planilla_Click(sender As Object, e As EventArgs) Handles btn_elimianr_planilla.Click
        ...
        If MessageBox.Show("Se va a eliminar el registro" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            Else
                ...
                DTRESULTADOS = PlanillaBl.EliminarPlanilla_Pagos_Proveedor(dgv_planillas.Item(0, dgv_planillas.CurrentRow.Index).Value)
                If DTRESULTADOS.Rows.Count() <> 0 Then
                    MsgBox("No se puede eliminar la Planilla, por poseer Pagos.", MsgBoxStyle.Exclamation)
                Else
                    btn_consultar_Click(sender, e)
                    dgv_planillas.Focus()
                End If
    ...
    Private Sub txtreferenciaSaldo_KeyDown(sender As Object, e As KeyEventArgs) Handles txtreferenciaSaldo.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            If txtreferenciaSaldo.Text = String.Empty Then
                Ayuda_TipoDoc(txtreferenciaSaldo)
                If Strings.Left(txtreferenciaSaldo.Text, 2) = "CD" Then
                    lbldetrac.Visible = True
                    dtp_detrac.Visible = True
                Else
                    lbldetrac.Visible = False
                    dtp_detrac.Visible = False
                End If
            Else
    ...
    Private Sub txtnumerosaldo_KeyPress(sender As Object, e As KeyPressEventArgs)
        If e.KeyChar = ChrW(Keys.Enter) Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub rb_comprobante_CheckedChanged(sender As Object, e As EventArgs) Handles rb_comprobante.CheckedChanged
        If rb_comprobante.Checked = True Then
            txt_cod_ane.Text = ""
            txt_des_ane.Text = ""
            txt_des_ane.Visible = False
            REMOVER_GRILLA()
        End If
    End Sub

    Private Sub REMOVER_GRILLA()
        If dgv_pendientes.RowCount() <> 0 Then
            For n As Integer = dgv_pendientes.Rows.Count - 1 To 0 Step -1
                Dim row As DataGridViewRow = dgv_pendientes.Rows(n)
                dgv_pendientes.Rows.Remove(row)
    ...
    Private Sub rb_Proveedor_CheckedChanged(sender As Object, e As EventArgs) Handles rb_Proveedor.CheckedChanged
        If rb_Proveedor.Checked = True Then
            ...
            gb_tipo_documento.Visible = False
            txt_des_ane.Focus()
        End If
    End Sub

    Private Sub Tool_Salir_Click_1(sender As Object, e As EventArgs) Handles Tool_Salir.Click
        Me.Close()
    End Sub
    ...
    Private Sub optexcel_Click(sender As Object, e As EventArgs) Handles optexcel.Click
        ...
        MostrarDocumentosPendiente_Proveedor()
        txtFiltro.Text = String.Empty
    End Sub
    Private Sub MostrarDocumentosPendiente_Proveedor()
        Try
            ...
            dtDocumentosPrincipal = clsLetrasBl.MostrandoProveedoresDocPendientes()
    ...
    Private Sub ImprimirDocumentosPendientes(ByVal _Codigo As String, ByVal _OPCION As Integer)
        Try
            ...
            If optrangocobro.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_PLanilla_Pagos(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    If optresumido.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_PAGOS_RESUMIDO.rpt", dtImprimir, "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    ElseIf optdetallado.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_PAGOS_DETALLADO.rpt", dtImprimir, "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    End If
                Else
                    MsgBox("No hay informacion disponible para mostrar.", MsgBoxStyle.Information)
                End If
            ElseIf optexcel.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_Plan_Pagos_Proveedor_Codigo(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    crystalBL.Muestra_Reporte("REP_PLA_PAGOS_PROVEEDOR.rpt", dtImprimir, "", "", "@codigo;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                Else
                    MsgBox("No hay informacion disponible para mostrar.", MsgBoxStyle.Information)
                End If
            End If
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        If ValidarUsuariosIngreso() = True Then
            ...
            ElseIf opt_planilla_detalle = "D" Then
                ...
                Item = dgv_detalles.Item(1, dgv_detalles.CurrentRow.Index).Value
                ...
                TipoDocRef = dgv_detalles.Item(11, dgv_detalles.CurrentRow.Index).Value
                NumDocRef = dgv_detalles.Item(12, dgv_detalles.CurrentRow.Index).Value
                    ...
                    If planillaBL.ActualizacionDocumentosEliminados_Pago_Proveedor(CodCliente, TipoDocRef, numeroDoc, MontodoC) = True Then
                        If TipoDocCobranza = "NC" Or TipoDocCobranza = "NCE" Then
                            ...
                            If dt_detalle.Rows.Count() <> 0 Then
                                ...
                                End If
                                planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE_PAY
                                If planillaBL.ActualizacionDocumentosEliminados_Pago_Proveedor(CodCliente, TipoDocCobranza, NumDocCobranza, (Math.Abs(CDbl(MontodoC))) * -1) = True Then
    ...
    Private Sub txtnumerosaldo_KeyDown(sender As Object, e As KeyEventArgs) Handles txtnumerosaldo.KeyDown
        If e.KeyCode = Keys.F1 Then Ayuda_Lista_Anticipos_Proveedor(txtnumerosaldo)
        If e.KeyCode = Keys.Enter Then
            'Dim dtValidacion As DataTable
            'clsFacturaBl = New ClsNegocio.RECEIVABLE
            'dtValidacion = New DataTable("Validacion")
            'dtValidacion = clsFacturaBl.Get_Validacion_Doc_Pago(Strings.Left(Trim(txtreferenciaSaldo.Text), 2), Trim(txtnumerosaldo.Text))
            'If dtValidacion.Rows.Count > 0 Then
            '    MessageBox.Show("El documento de Referencia ingresado, se encuentra " & vbNewLine & "registrado en el Sistema!", "Sistemas", MessageBoxButtons.OK, MessageBoxIcon.Information)
            '    If Strings.Left(txtreferenciaSaldo.Text, 2) = "CD" Then
            '        dtp_detrac.Focus()
            '    Else
            ...
            tipo_doc = TraerCodigo(txtreferenciaSaldo.Text).Trim
            If txtnumerosaldo.Text = "" And tipo_doc = "ANP" Then
                Ayuda_Lista_Anticipos_Proveedor(txtnumerosaldo)
            Else
                btn_grabar_det.Focus()
            End If
    ...
    Private Sub Ayuda_Lista_Anticipos_Proveedor(ByVal texto As TextBox)
        Try
            Dim frm As New FrmBuscar_Anticipos
            Dim sql As String = String.Empty
            sql = "CO_SP_S_ANTICIPOS_PAY"
            frm.CadenaConsulta = sql
            frm._Flag_Filtro = True
            frm.Filtros1 = txt_cod_ane.Text.Trim
            frm.Titulo = "Lista de Anticipos de Proveedor"
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
                texto.Text = frm.Data_Matriz.Rows(0).Item(1).ToString & frm.Data_Matriz.Rows(0).Item(2).ToString
                _serie_anticipos = frm.Data_Matriz.Rows(0).Item(1).ToString
                _numero_anticipos = frm.Data_Matriz.Rows(0).Item(2).ToString
            End If
            frm.Close()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub txtEfectoNumerodocRef_KeyDown(sender As Object, e As KeyEventArgs) Handles txtEfectoNumerodocRef.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            clsFacturaBl = New ClsNegocio.RECEIVABLE
            dtValidacion = New DataTable("Validacion")
            dtValidacion = clsFacturaBl.Get_Validacion_Doc_Pago(Strings.Left(Trim(txt_efectivo_doc_Ref.Text), 2), Trim(txtEfectoNumerodocRef.Text))
            If dtValidacion.Rows.Count > 0 Then
    ...
    Private Sub txt_num_ref_ban_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_num_ref_ban.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            'clsFacturaBl = New ClsNegocio.RECEIVABLE
            'dtValidacion = New DataTable("Validacion")
            'dtValidacion = clsFacturaBl.Get_Validacion_Doc_Pago(Strings.Left(Trim(txtdocRefBancos.Text), 2), Trim(txt_num_ref_ban.Text))
            'If dtValidacion.Rows.Count > 0 Then
            '    MessageBox.Show("El documento de Referencia ingresado, se encuentra " & vbNewLine & "registrado en el Sistema, ingrese el número correcto!", "Sistemas", MessageBoxButtons.OK, MessageBoxIcon.Information)
            '    'txt_num_ref_ban.Text = ""
            '    txt_gastos_financieros.Focus()
            'Else
            txt_gastos_financieros.Focus()
        End If
    End Sub
    ...
    Private Sub txt_num_ref_ban_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_num_ref_ban.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Then txt_itf.Focus() ' SendKeys.Send("{TAB}")
    End Sub
    ...
    Private Sub txtnumerosaldo_KeyPress_1(sender As Object, e As KeyPressEventArgs) Handles txtnumerosaldo.KeyPress
        'If e.KeyChar = ChrW(Keys.Enter) Then
        '    If Strings.Left(txtreferenciaSaldo.Text, 2) = "CD" Then
        '        dtp_detrac.Focus()
        '    Else
    ...
    Private Sub dtp_detrac_KeyPress(sender As Object, e As KeyPressEventArgs) Handles dtp_detrac.KeyPress
        If e.KeyChar = ChrW(Keys.Enter) Then btn_grabar_det.Focus() ' SendKeys.Send("{TAB}")
    End Sub

    Private Sub dtp_detrac_KeyDown(sender As Object, e As KeyEventArgs) Handles dtp_detrac.KeyDown
        If e.KeyCode = Keys.Enter Then
            btn_grabar_det.Focus()
        End If
    End Sub
    ...
    Private Sub RadioButton1_CheckedChanged(sender As Object, e As EventArgs) Handles RadioButton1.CheckedChanged
        If RadioButton1.Checked = True Then
            ...
            dtpinicio_fecha_pago.Value = CDate(mtb_fec_cob.Text) 'ModFunciones.Primer_Dia_Mes(CDate(FechaActualPlanilla).ToString("dd/MM/yyyy"))
            dtpfecha_fin_pago.Value = CDate(mtb_fec_cob.Text) 'ModFunciones.Ultimo_Dia_Mes(CDate(FechaActualPlanilla).ToString("dd/MM/yyyy"))
            dtpinicio_fecha_pago.Enabled = True
            dtpfecha_fin_pago.Enabled = True
            Button5.Select()
        End If
    End Sub

    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        Ayuda_Pagos_Fecha(dtpinicio_fecha_pago.Value.ToString("dd/MM/yyyy"), dtpfecha_fin_pago.Value.ToString("dd/MM/yyyy"))
    End Sub

    Private Sub dtpinicio_fecha_pago_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpinicio_fecha_pago.KeyDown
        If e.KeyCode = Keys.Enter Then
            dtpfecha_fin_pago.Select()
        End If
    End Sub

    Private Sub dtpfecha_fin_pago_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpfecha_fin_pago.KeyDown
        If e.KeyCode = Keys.Enter Then
    ...
    Private Sub txtserieDoc_KeyDown(sender As Object, e As KeyEventArgs) Handles txtserieDoc.KeyDown
        If e.KeyCode = Keys.F1 Then Ayuda_Aplicacion_NC()
        If e.KeyCode = Keys.Enter Then
            If txtserieDoc.Text = String.Empty Then
                Ayuda_Aplicacion_NC()
            Else
    ...
    Private Sub txtnumDoc_KeyDown(sender As Object, e As KeyEventArgs) Handles txtnumDoc.KeyDown
        ...
        End If
    End Sub
    Private Sub Ayuda_Aplicacion_NC(Optional ByVal _CodigoCuentaContable As String = "")
        Try
            If _CodigoCuentaContable = String.Empty Then
                Dim frm As New frmBuscar
                Dim sql As String = String.Empty
                sql = "CO_SP_S_APLIC_NC"
                frm.CadenaConsulta = sql
                frm._Flag_Filtro = True
                frm.Filtros1 = txt_cod_ane.Text.Trim
                frm.Titulo = "Notas de Credito del Proveedor"
                frm.ShowDialog()
                If frm.Data_Matriz.Rows.Count > 0 Then
                    txtserieDoc.Text = frm.Data_Matriz.Rows(0).Item(1).ToString
                    txtnumDoc.Text = frm.Data_Matriz.Rows(0).Item(2).ToString
                    txtmonto.Text = frm.Data_Matriz.Rows(0).Item(4).ToString
    ...
    Private Sub txt_gastos_financieros_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_gastos_financieros.KeyPress
    ...
    Private Sub txt_gastos_financieros_Leave(sender As Object, e As EventArgs) Handles txt_gastos_financieros.Leave
        If txt_gastos_financieros.Text = "" Then
            txt_gastos_financieros.Text = Format(Double.Parse(0), "##,##0.00")
        Else
            txt_gastos_financieros.Text = Format(Double.Parse(txt_gastos_financieros.Text), "##,##0.00")
        End If
    End Sub

    Private Sub txt_gastos_financieros_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_gastos_financieros.KeyDown
        If e.KeyCode = Keys.Enter Then txt_gastos_bancarios.Focus() ' SendKeys.Send("{TAB}")
    End Sub
    ...
    Private Sub txt_itf_Leave(sender As Object, e As EventArgs) Handles txt_itf.Leave
        If txt_itf.Text = "" Then
            txt_itf.Text = Format(Double.Parse(0), "##,##0.00")
        Else
            txt_itf.Text = Format(Double.Parse(txt_itf.Text), "##,##0.00")
        End If
    End Sub

    Private Sub txtnumerosaldo_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles txtnumerosaldo.MouseDoubleClick
        Ayuda_Lista_Anticipos_Proveedor(txtnumerosaldo)
    End Sub
    ...
    Private Sub Sumatoria_Planilla_Referencia()
        Try
            ...
            dt_detalle_sumatoria = planillaBL.get_planilla_Sumatoria_Ref_Pago_Proveedor(num_planilla)
    ...
    Private Sub txt_gastos_bancarios_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_gastos_bancarios.KeyPress
        If Char.IsDigit(e.KeyChar) Or e.KeyChar = "." Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txt_gastos_bancarios_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_gastos_bancarios.KeyDown
        If e.KeyCode = Keys.Enter Then txt_ing_redondeo.Focus() ' SendKeys.Send("{TAB}")
    End Sub

    Private Sub txt_gastos_bancarios_Leave(sender As Object, e As EventArgs) Handles txt_gastos_bancarios.Leave
        If txt_gastos_bancarios.Text = "" Then
            txt_gastos_bancarios.Text = Format(Double.Parse(0), "##,##0.00")
        Else
            txt_gastos_bancarios.Text = Format(Double.Parse(txt_gastos_bancarios.Text), "##,##0.00")
        End If
    End Sub

    Private Sub txt_ing_redondeo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_ing_redondeo.KeyPress
        If Char.IsDigit(e.KeyChar) Or e.KeyChar = "." Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txt_ing_redondeo_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_ing_redondeo.KeyDown
        If e.KeyCode = Keys.Enter Then
            txt_gastos_redondeo.Focus()
        End If
    End Sub

    Private Sub txt_gastos_redondeo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_gastos_redondeo.KeyPress
        If Char.IsDigit(e.KeyChar) Or e.KeyChar = "." Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
    ...
    Private Sub txt_gastos_redondeo_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_gastos_redondeo.KeyDown
        If e.KeyCode = Keys.Enter Then
            btn_grabar_det.Focus()
        End If
    End Sub

    Private Sub txt_ing_redondeo_Leave(sender As Object, e As EventArgs) Handles txt_ing_redondeo.Leave
        If txt_ing_redondeo.Text = "" Then
            txt_ing_redondeo.Text = Format(Double.Parse(0), "##,##0.00")
        Else
            txt_ing_redondeo.Text = Format(Double.Parse(txt_ing_redondeo.Text), "##,##0.00")
        End If
    End Sub

    Private Sub txt_gastos_redondeo_Leave(sender As Object, e As EventArgs) Handles txt_gastos_redondeo.Leave
        If txt_gastos_redondeo.Text = "" Then
            txt_gastos_redondeo.Text = Format(Double.Parse(0), "##,##0.00")
        Else
            txt_gastos_redondeo.Text = Format(Double.Parse(txt_gastos_redondeo.Text), "##,##0.00")
        End If
    End Sub

    Private Sub RadioButton2_CheckedChanged(sender As Object, e As EventArgs) Handles rb_tipo_documento.CheckedChanged
        If rb_tipo_documento.Checked = True Then
            txt_cod_ane.Text = ""
            txt_des_ane.Text = ""
            txt_tipo_documento.Text = ""
            txt_des_tipo_documento.Text = ""
            mtb_total_cobMN.Text = 0
            mtb_total_cobME.Text = 0
            REMOVER_GRILLA()
            GroupBox11.Visible = False
            gb_tipo_documento.Visible = True
            txt_tipo_documento.Focus()
        End If
    End Sub

    Private Sub txt_tipo_documento_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles txt_tipo_documento.MouseDoubleClick
        Ayuda_Tipo_Documento()
    End Sub

    Private Sub txt_tipo_documento_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_tipo_documento.KeyDown
        If e.KeyCode = Keys.F1 Then
            Ayuda_Tipo_Documento()
        End If
        If e.KeyCode = Keys.Enter Then
            If txt_tipo_documento.Text = "" Then
                Ayuda_Tipo_Documento()
            End If
        End If
    End Sub

    Private Sub Ayuda_Tipo_Documento()
        Try

            Dim frm As New frmBuscar
            Dim sql As String = String.Empty
            sql = "CX_SP_S_CUSTOMER_BALANCE_PAY"
            frm.CadenaConsulta = sql
            frm.Titulo = "Tipo y Número de Documento"
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
                txt_tipo_documento.Text = frm.Data_Matriz.Rows(0).Item(0).ToString
                txt_des_tipo_documento.Text = frm.Data_Matriz.Rows(0).Item(1).ToString
                If txt_tipo_documento.Text <> String.Empty Then
                    Cargar_Documentos_Pendientes_Tipo_Documento()
                    dgv_pendientes.Focus()
                End If
            End If
            frm.Close()
    ...
    Private Sub chk_comprobante_retencion_CheckedChanged(sender As Object, e As EventArgs) Handles chk_comprobante_retencion.CheckedChanged
        If chk_comprobante_retencion.Checked Then
            lbl_fecha_cr.Visible = True
            dtp_fecha_cr.Visible = True
            dtp_fecha_cr.Value = Date.Now
        Else
            lbl_fecha_cr.Visible = False
            dtp_fecha_cr.Visible = False
