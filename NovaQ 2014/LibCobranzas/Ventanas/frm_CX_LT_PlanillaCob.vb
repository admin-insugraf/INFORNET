...
Public Class frm_CX_LT_PlanillaCobs
...
#Region "subprocesos"
    ...
    Private Sub Sumar_Marcados()
        ...
        Else
            For i As Integer = 0 To dgv_pendientes.Rows.Count - 1
                If dgv_pendientes.Rows(i).Cells("chk_Sel").Value = True Then
                    If txt_cod_mon.Text = "MN" Then 'dgv_pendientes.Rows(i).Cells("COL_COD_MON").Value = "MN" Then
                        totalMN += dgv_pendientes.Rows(i).Cells("COL_SALDO").Value
                    ElseIf txt_cod_mon.Text = "ME" Then 'dgv_pendientes.Rows(i).Cells("COL_COD_MON").Value = "ME" Then
                        totalME += dgv_pendientes.Rows(i).Cells("COL_SALDO").Value
                    End If
                End If
            Next
    ...
    Private Sub Cargar_Documentos()
        ...
        cmb_tipo_doc.DataSource = documentosBL.get_DocumentosNC()
    ...
    Private Sub Cargar_Tipo_Cobranza()
        ...
        cmb_tipo_cob.DataSource = tipoCobranzaBL.get_Tipos(GRUPO_COBRANZA)
    ...
    Private Sub Cargar_Documentos_Pendientes()
        ...
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes(txt_cod_ane.Text, _fecha)
    ...
    Private Sub Cargar_Documentos_Pendientes_x_Fechas(ByVal fecha_inicial As String, ByVal fecha_final As String)
        Dim saldosBL As New ClsOperaciones.CUSTOMER_BALANCE
        dgv_pendientes.DataSource = saldosBL.get_Documentos_Pendientes_x_Clientes_x_Fechas(fecha_inicial, fecha_final)
        saldosBL = Nothing
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
            dgv_pendientes.Rows(i).Cells("COL_SALDO").Value = dgv_pendientes.Rows(i).Cells("COL_MONTO_SALDO").Value
        Next
    ...
    Private Sub Consultar_Planillas_Cab_xfecha()
        ...
        dtvC = planillaBL.get_planilla_x_fecha(fecha_ini, fecha_fin, GRUPO_COBRANZA).DefaultView
...
#End Region
    ...
    Private Sub btn_crear_planilla_Click(sender As Object, e As EventArgs) Handles btn_crear_planilla.Click
        ...
        dtTipoCamb_tc = cl.Verifircar_Planilla_Cobros(dtp_fec_nueva_pla.Value.ToString("dd/MM/yyyy"), GRUPO_COBRANZA)
        If dtTipoCamb_tc.Rows.Count() > 0 Then
            MsgBox("Existe una planilla con la fecha elegida, proceda a modificar dicha planilla.", MsgBoxStyle.Critical, "Sistemas")
            Exit Sub
        End If
        ...
        With planillaBE
            ...
            .GRUPO_COBRANZA = GRUPO_COBRANZA
        End With
        planillaBL.Insert(planillaBE, True, numero_planilla)
        planillaBE = Nothing
        planillaBL = Nothing
    ...
    Private Sub btn_grabar_det_Click(sender As Object, e As EventArgs) Handles btn_grabar_det.Click
        ...
        For i As Integer = 0 To dgv_pendientes.RowCount - 1
            ...
            If dgv_pendientes.Rows(i).Cells("chk_Sel").Value Then
                    ...
                    ElseIf cmb_tipo_doc.SelectedValue = "FP" Then 'Or cmb_tipo_doc.SelectedValue = "CR" Then
                        If txt_cod_mon.Text = "MN" Then
                            If Math.Abs(CDbl(txtmonto.Text)) > Math.Abs(CDbl(mtb_total_cobMN.Text)) Then
                                ...
                                Exit Sub
                            End If
                        ElseIf txt_cod_mon.Text = "ME" Then
                            If Math.Abs(CDbl(txtmonto.Text)) > Math.Abs(CDbl(mtb_total_cobME.Text)) Then
                                MessageBox.Show("El monto a aplicar es MAYOR al saldo del documento a cobrar. Verifique!", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                                Exit Sub
                            End If
                        End If
                ...
                With planilladetBE
                    ...
                    If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
                        If cmb_tipo_doc.SelectedValue = "NC" Or cmb_tipo_doc.SelectedValue = "FP" Then
                            .AMOUNT = dgv_pendientes.Rows(i).Cells("COL_SALDO").Value.ToString '(Math.Abs(CDbl(txtmonto.Text)))
                        Else
                    ...
                    If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
                        ...
                        ElseIf cmb_tipo_doc.SelectedValue = "FP" Or cmb_tipo_doc.SelectedValue = "CR" Then
                            .DOC_REF = tipo_doc_aplica_fp 'cmb_tipo_doc.SelectedValue.ToString
                    ...
                    If txt_gastos_financieros.Text = "" Then
                        .AMOUNT_GASTOS_FINANCIEROS = CDbl(0)
                    Else
                        .AMOUNT_GASTOS_FINANCIEROS = CDbl(txt_gastos_financieros.Text)
                    End If
                ...
                If gb_aplidoc.Enabled = True And gb_banco.Enabled = False Then
                    'If cmb_tipo_doc.SelectedValue = "NC" Then 'Or cmb_tipo_doc.SelectedValue = "FP" Then
                    '    Exit For
                    'End If
                End If
            End If
    ...
    Private Sub cmb_tipo_cob_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_tipo_cob.SelectedIndexChanged
        ...
        If bol_activar_cmbs Then
            If cmb_tipo_cob.SelectedIndex <> -1 Then
                ...
                txt_itf.Text = "0.0"
                txt_gastos_financieros.Text = "0.0"
                ...
                If tipocobBE.IS_APPL = 1 And tipocobBE.TRANS_TYPE = 3 Then 'aplicacion de NC o FP
                ...
                Else
                    ...
                    If dgv_pendientes.Rows(i).Cells("COL_DOCUMENT_ID").Value <> "NC" Then
                        If CURRENCY_ID = "ME" Then
                            If MonedaCobranza = "ME" Then
                                AMOUNT = AMOUNT
                            Else
                                AMOUNT = Math.Round(AMOUNT * tipo_cambio_planilla, 6) 'dgv_pendientes.Rows(i).Cells("COL_TC").Value
                            End If
                    ...
                    Else
                        If CURRENCY_ID = "ME" Then
                            ...
                            Else
                                AMOUNT = Math.Round((Math.Abs(AMOUNT * tipo_cambio_planilla) * -1), 6) 'dgv_pendientes.Rows(i).Cells("COL_TC").Value) * -1)
                            End If
                        ElseIf CURRENCY_ID = "MN" Then
                            If MonedaCobranza = "MN" Then
                                AMOUNT = (Math.Abs(AMOUNT) * -1)
                        ...
                        End If
                    End If
                    dgv_pendientes.Rows(i).Cells("COL_SALDO").Value = IIf(String.IsNullOrEmpty(AMOUNT), 0, Math.Round(AMOUNT, 6))
                    dgv_pendientes.Update()
                End If
            Next
            Call Sumar_Marcados()
    ...
    Private Sub Editar_Planilla()
        ...
        If dt_detalle.Rows.Count() <> 0 Then
            ...
            dgv_detalles.Columns(23).Width = 180
            ...
            dgv_detalles.Columns(24).Width = 180
            ...
            dgv_detalles.Columns(27).Visible = True
            ...
            dgv_detalles.Columns(29).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            dgv_detalles.Columns(29).HeaderText = "Gastos Financieros"

            dgv_detalles.Columns("IngresosPorRedondeo").DefaultCellStyle.Format = "##,##0.00"
            dgv_detalles.Columns("IngresosPorRedondeo").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            dgv_detalles.Columns("IngresosPorRedondeo").HeaderText = "Ingresos por Redondeo"

            dgv_detalles.Columns("GastosPorRedondeo").DefaultCellStyle.Format = "##,##0.00"
            dgv_detalles.Columns("GastosPorRedondeo").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            dgv_detalles.Columns("GastosPorRedondeo").HeaderText = "Gastos por Redondeo"
            ...
            Else
                ...
                If dt_detalle.Rows.Count() <> 0 Then
                    ...
                    If TipoD_Cartera <> "NC" Then
                        If Moneda_Cartera = "ME" Then
                            If MonDoc = "ME" Then
                                MontodoC = MontodoC
                            Else
                                MontodoC = Math.Round(MontodoC / Tipo_Cambio_Detalle, 4) ' ClsVarComun.TCCompra
                            End If
                        ElseIf Moneda_Cartera = "MN" Then
                            If MonDoc = "MN" Then
                                MontodoC = MontodoC
                            Else
                                MontodoC = Math.Round(MontodoC * Tipo_Cambio_Detalle, 4) ' ClsVarComun.TCVenta
                            End If
                        End If
                    Else
                        If Moneda_Cartera = "ME" Then
                            ...
                            Else
                                MontodoC = Math.Round(Math.Abs(MontodoC / Tipo_Cambio_Detalle), 4) * -1 ' ClsVarComun.TCCompra)) * -1
                            End If
                        ElseIf Moneda_Cartera = "MN" Then
                            If MonDoc = "MN" Then
                                MontodoC = (Math.Abs(MontodoC)) * -1
                            Else
                                MontodoC = Math.Round(Math.Abs(MontodoC * Tipo_Cambio_Detalle), 4) * -1 ' ClsVarComun.TCVenta)) * -1
                            End If
                        End If
                    End If
                    planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE
                    If planillaBL.ActualizacionDocumentosEliminados(CodCliente, tipoDoc, numeroDoc, MontodoC) = True Then
                            ...
                            If dt_detalle.Rows.Count() <> 0 Then
                                ...
                                If MonedaNC = "ME" Then
                                    If MonDoc = "ME" Then
                                        MontodoC = CDbl(dgv_detalles.CurrentRow.Cells("AMOUNT").Value)
                                    Else
                                        MontodoC = Math.Round(CDbl(dgv_detalles.CurrentRow.Cells("AMOUNT").Value) / Tipo_Cambio_Detalle, 4) ' ClsVarComun.TCCompra
                                    End If
                                ElseIf MonedaNC = "MN" Then
                                    If MonDoc = "MN" Then
                                        MontodoC = CDbl(dgv_detalles.CurrentRow.Cells("AMOUNT").Value)
                                    Else
                                        MontodoC = Math.Round(CDbl(dgv_detalles.CurrentRow.Cells("AMOUNT").Value) * Tipo_Cambio_Detalle, 4) ' ClsVarComun.TCVenta
                                    End If
                                End If
                                planillaBL = New ClsOperaciones.RECEIVABLE_LIST_LINE
                        ...
                        Else
                            MsgBox("Error en la eliminación.", MsgBoxStyle.Critical)
                        End If
                    Else
    ...
    Private Function VerficarNotaC() As Boolean
        Try
            ...
            If dt_detalle.Rows.Count() <> 0 Then
                ...
                mtb_fecha_aplic_doc.Text = dt_detalle.Rows(0).Item("DOC_DATE").ToString
                txtmonto.Select()
            Else
        ...
        Try
            ...
            Else
                ...
                If codigo = String.Empty Then
                    MsgBox("Es necesario una numeración.", MsgBoxStyle.Exclamation)
                    Exit Sub
                End If
                ...
                dtcodigo = ClsasientoBl.ValidarCodigo_Cobranzas("03", codigo, CDate(_FECHA_PLANILLA).Year, CDate(_FECHA_PLANILLA).Month)
            ...
            If ClsCobranzaBl.Contabilizar_Caja_Cobranzas(_CODIGO_PLANILLA, CDate(_FECHA_PLANILLA), _NUMERACION, ProgressBar1) = True Then
                ProgressBar1.Value = 0
                btn_consultar_Click(sender, e)
                MsgBox("Planilla contabilizada correctamente.", MsgBoxStyle.Information)
        ...
        Else
            If cmb_tipo_doc.SelectedValue <> "FP" Then
                If txtnumDoc.Text.Trim().Length > 0 Then
                    txtnumDoc.Text = txtnumDoc.Text.PadLeft(7, Char.Parse("0"))
                End If
            End If
        End If
    End Sub
    ...
    Private Sub ImprimirDocumentosPendientes(ByVal _Codigo As String, ByVal _OPCION As Integer)
        Try
            ...
            If optrangocobro.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_PLanilla_Cobranza(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    If optresumido.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_COB_RESUMIDO.rpt", "", "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    ElseIf optdetallado.Checked = True Then
                        crystalBL.Muestra_Reporte("REP_PLA_COB_DETALLADO.rpt", "", "", "", "@cod;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                    End If
                Else
            ...
            ElseIf optexcel.Checked = True Then
                ...
                dtImprimir = reporteBL.EjecutarReporte_Plan_Cob_Cliente_Codigo(_Codigo, _OPCION)
                dtDatosCompany = reporteBL.Obtener_Datos_Empresa()
                If dtImprimir.Rows.Count() <> 0 Then
                    crystalBL.Muestra_Reporte("REP_PLA_COB_CLIENTE.rpt", "", "", "", "@codigo;" & _Codigo, "@TIP;" & _OPCION, "EMPRESA;" & dtDatosCompany.Rows(0).Item("REPORT_SCREEN").ToString, "RUC;" & dtDatosCompany.Rows(0).Item("VAT_REGISTRATION").ToString)
                Else
                    MsgBox("No hay informacion disponible para mostrar.", MsgBoxStyle.Information)
                End If
            End If
    ...
    Private Sub Sumatoria_Planilla_Referencia()
        Try
            ...
            dt_detalle_sumatoria = planillaBL.get_planilla_Sumatoria_Ref_Pago_Cliente(num_planilla)
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        Cargar_Documentos_Pendientes_x_Fechas(dtpinicio_fecha_pago.Value.ToString("dd/MM/yyyy"), dtpfecha_fin_pago.Value.ToString("dd/MM/yyyy"))
    End Sub

    Private Sub rb_Proveedor_CheckedChanged(sender As Object, e As EventArgs) Handles rb_Cliente.CheckedChanged
    ...
    Private Sub txt_gastos_financieros_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_gastos_financieros.KeyDown
        If e.KeyCode = Keys.Enter Then
            txt_ing_redondeo.Focus()
        End If
    End Sub

    Private Sub TextBox3_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_gastos_redondeo.KeyPress, txt_ing_redondeo.KeyPress
        If Char.IsDigit(e.KeyChar) Or e.KeyChar = "." Then
            e.Handled = False
        ElseIf Char.IsControl(e.KeyChar) Then
            e.Handled = False
