...
Public Class FrmChequesPagos
    ...
    Private Sub FrmChequesPagos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub GenerarColummnaDataTable()
        Try
            ...
            dgvDocumentos.Columns(2).Width = 150
    ...
    Private Sub Ayuda_Clientes(Optional ByVal _CodigoCliente As String = "")
        Try
            If _CodigoCliente = String.Empty Then
                ...
                sql = "COMPRAS_SP_S_CHEQUES_PAGOS_VENDOR"
            ...
            Else
                ...
                dtDetalleLetra = clsLetrasBl.Get_ManualProveedores(_CodigoCliente)
    ...
    Private Sub MostrarDocumentosPendiente()
        Try
            ...
            dtDocumentosPrincipal = clsLetrasBl.Get_DocumentosPendientesChequesPago(_CodigoCliente)
            If dtDocumentosPrincipal.Rows.Count() <> 0 Then
                ...
                dgvDocumentos.Focus()
            Else
                MsgBox("El Proveedor no posee documentos para Generar un Cheque.", MsgBoxStyle.Information)
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub DocumentosPlanilla()
        Try
            ...
            dtv = New DataView
            dtDocumentoPlanilla = clsLetrasBl.Get_DocumentosPlanillaChequesPago(dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value, cboopcionesBusqueda.SelectedIndex)
    ...
    Private Sub DocumentosLetras(ByVal _CodigoPlanilla As String)
        Try
            ...
            var = String.Empty
            dtDocumentoLetra = clsLetrasBl.Get_DocumentosChequesPagos(_CodigoPlanilla)
            dgvLetra.DataSource = Nothing
    ...
    Private Sub DocumentosInciales(ByVal _CodigoPlanilla As String)
        Try
            ...
            dtDocumentoLetra = New DataTable
            var = String.Empty
            dtDocumentoLetra = clsLetrasBl.Get_DocumentosLetrasPagos(_CodigoPlanilla)
            dgvLetra.DataSource = Nothing
            If dtDocumentoLetra.Rows.Count() <> 0 Then
                dgvLetra.DataSource = dtDocumentoLetra
                For i As Integer = 0 To dtDocumentoLetra.Rows.Count() - 1
   ...
   ByVal e As DataGridViewEditingControlShowingEventArgs) Handles dgvDocumentos.EditingControlShowing
    ...
    Sub AgregarLetra(ByVal NLetras As Integer)
        Try
            ...
            Dim x As Integer = 0
            clsLetraCorrelativoBl = New ClsTransacciones.clsLetrasCorrelativo
            clsLetraCorrelativoBl.Get_NumeroCorrelativoLetra("CHP")
        ...
        If e.ColumnIndex = 1 Then
            If dgvDetalleLetra.Columns(e.ColumnIndex).Name = "Nro_Letra" Then
                ...
                Else
                    ...
                    CodigoLetra = CodigoLetra.PadLeft(10, Char.Parse("0"))
                    ''Verificar NumeracionLetra
                    clsLetrasBl = New ClsNegocio.LETTER_EXCHANGE
                    dtDetalleLetra = New DataTable("NumeracionLetra")
                    dtDetalleLetra = clsLetrasBl.Get_VerificacionNumeracionCheque(Strings.Right(CodigoLetra, 10))
                    If dtDetalleLetra.Rows.Count() <> 0 Then
                        MsgBox("La Numeracion del Cheque ya existe, Verifique.", MsgBoxStyle.Information)
                        e.Cancel = True
                    End If
                End If
            End If
    ...
    Private Sub GuardarLetra()
        Try
            ...
            'numeroCorrelativo
            clsLetraCorrelativoBl = New ClsTransacciones.clsLetrasCorrelativo
            clsLetraCorrelativoBl.Get_NumeroCorrelativoChequePagos()
            With letraCabEnt
            ...
            For Each rows As DataGridViewRow In dgvDetalleLetra.Rows
                ...
                NumMax = NumMax.PadLeft(10, Char.Parse("0"))
                listaNumeroMax.Add(Strings.Right(NumMax, 10))
            Next
            ...
            Me.Cursor = Cursors.WaitCursor
            If clsLetrasBl.InsertChequePagos(letraCabEnt, lista_det, lista_det_Cheque_Cartear, dtDocumentosPrincipal, TipoNumeracion, listaNumeroMax.Max(), cmb_moneda.SelectedValue, PorcPercepcion, Flag_Retencion) Then
                msj = String.Empty
                msj = "Cheque generado Correctamente, Nro Documento: " & letraCabEnt.ID
    ...
    Private Function validaciones() As Boolean
        ...
        Try
            ...
            Else
                clsLetrasBl = New ClsNegocio.LETTER_EXCHANGE
                dtDetalleLetra = New DataTable("Clientes")
                dtDetalleLetra = clsLetrasBl.Get_ManualProveedores(txtCodCliente.Text)
                If dtDetalleLetra.Rows().Count() = 0 Then
                    MsgBox("El codigo ingresado del Proveedor es incorrecto.", MsgBoxStyle.Critical)
                    estado = False
                    txtCodCliente.Focus()
                    Exit Try
                End If
            ...
            If dgvDocumentos.Rows.Count() = 0 Then
                MsgBox("El Proveedor no posee Documentos para Generar un Canje por Cheque.", MsgBoxStyle.Exclamation)
                estado = False
                txtCodCliente.Focus()
                Exit Try
            End If
            If CDbl(txttotalMN.Text) = 0 Or CDbl(txttotalME.Text) = 0 Then
                MsgBox("No se puede Generar un Canje de Cheques con montos iguales a  Cero (0).", MsgBoxStyle.Exclamation)
                estado = False
                Exit Try
            End If
        ...
        Try
            ...
            If cmb_moneda.SelectedValue.ToString = "MN" Then
                If Val(txtTotalLetraMN.Text) <> contador_monto Then
                    MsgBox("La suma de la(s) cheque(s) no son iguales al monto total a canjear.", MsgBoxStyle.Critical, "Aviso")
                    estado = False
                    Exit Try
                End If
            ElseIf cmb_moneda.SelectedValue.ToString = "ME" Then
    ...
    Private Sub BtnGuardar_Click(sender As Object, e As EventArgs) Handles BtnGuardar.Click
        ...
        If MessageBox.Show("Se va a proceder a la Creacion de un Nuevo Documento" & vbCrLf & "¿Desea continuar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            Else
                ...
                For Each rows As DataGridViewRow In dgvDetalleLetra.Rows
                    If LetraBl.ConsultandoDocExisteChequePagos(rows.Cells("Nro_Letra").Value, txtCodCliente.Text) = False Then
  ...
  ByVal e As System.Windows.Forms.KeyPressEventArgs)
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        If MessageBox.Show("Se va a eliminar el registro" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            ...
            Dim LetraBl As New ClsOperaciones.LETTER_EXCHANGE
            Me.Cursor = Cursors.WaitCursor
            If LetraBl.ConsultandoDocEliminacionChequePagos(CodigoPlanilla, Usuario) Then
                'Dim msj As String = String.Empty
                msj = "Documento Eliminado Correctamente, Nro Documento: " & CodigoPlanilla
                MsgBox(msj, MsgBoxStyle.Information)
            End If
            Me.Cursor = Cursors.Default
            DocumentosPlanilla()
    ...
    Private Sub btn_anular_Click(sender As Object, e As EventArgs) Handles btn_anular.Click
        ...
        If MessageBox.Show("Se va a anular el registro" & vbCrLf & "¿Desea continuar?", "Anular", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            Dim LetraBl As New ClsOperaciones.LETTER_EXCHANGE
            Me.Cursor = Cursors.WaitCursor
            If LetraBl.ConsultandoDocAnulacionChequePagos(CodigoPlanilla, LibComunVar.ClsVarComun.USUARIO) = True Then
