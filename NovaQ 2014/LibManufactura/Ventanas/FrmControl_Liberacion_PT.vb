...
Public Class FrmControl_Liberacion_PT
    ...
    Public _estado_control As String = ""
    ...
    Private Sub ModificandoArticulos()
        Try
            'GbCabecera.Enabled = False
            Dim frmArticulo As New FrmControl_Liberacion_Modificar_Prod_PT
            Dim lotes As String
            ...
            frmArticulo.txtlote.Text = lotes
    ...
    Private Sub txtFiltro_TextChanged(sender As Object, e As EventArgs) Handles txtFiltro.TextChanged
        ...
        Else
            ...
            If dtv.Count() = 0 Then
                ...
                If dtv.Count() = 0 Then
                    STRorden = String.Empty
                    STRorden = dgvCabeceraAlmacen.Columns(8).Name
                    dtv.RowFilter = STRorden & " like '%" & txtFiltro.Text & "%'"
                    If dtv.Count() = 0 Then
                        STRorden = String.Empty
                        STRorden = dgvCabeceraAlmacen.Columns(3).Name & "+" & dgvCabeceraAlmacen.Columns(8).Name
                        lblCantidad.Text = "Se encontraron " & dtv.Count & " registros"
                        Exit Sub
                    End If
                End If
    ...
    Private Sub MostrandoDatosGeneralesCabeceraAlmacen()
        Try
            ...
            dtData = clsBusquedaBl.Documentos_Liberacion_PT(dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value.ToString("dd/MM/yyyy"), _
                                                                         cboopcionesBusqueda.SelectedIndex, "I", "NI", "'04'", cboEstados.Text)
            ...
            If dtData.Rows.Count() <> 0 Then
                ...
                dgvCabeceraAlmacen.Columns(7).DisplayIndex = 2
                dgvCabeceraAlmacen.Columns(8).DisplayIndex = 3
                STRorden = dgvCabeceraAlmacen.Columns(3).Name & "+" & dgvCabeceraAlmacen.Columns(8).Name '' & "+" & dgvCabeceraAlmacen.Columns(2).Name & "+" & dgvCabeceraAlmacen.Columns(3).Name
                lblCantidad.Text = "Se encontraron " & dtv.Count & " registros"
        ...
        Try
            ...
            If dtCabeceraAlm.Rows.Count() <> 0 Then
                For i As Integer = 0 To dtCabeceraAlm.Rows.Count() - 1
                    dtpFecha.MinDate = CDate(dtCabeceraAlm.Rows(i).Item("DATE_DOCUMENT"))
                ...
                dtDetalleAlm = clsAlmacenDetBl.Detalle_Almacen_Control_PT(idAlmacen, documentId, NumeroDocument, ESTADO)
                dgvDetalle.DataSource = Nothing
                dgvDetalle.Rows.Clear()
                If dtDetalleAlm.Rows.Count() <> 0 Then
                    ...
                    For j As Integer = 0 To dtDetalleAlm.Rows.Count() - 1
                        ...
                        dgvDetalle.Rows.Add(dtDetalleAlm.Rows(j).Item("ITEM").ToString, dtDetalleAlm.Rows(j).Item("PART_ID").ToString, dtDetalleAlm.Rows(j).Item("PART_DESCRIPTION").ToString, _
                           ...
                           dtDetalleAlm.Rows(j).Item("QT_BULTOS").ToString, IIf(dtDetalleAlm.Rows(j).Item("OPT_ENTREGA").ToString = "0", "Parcial", "Total"), "", dtDetalleAlm.Rows(j).Item("NUMBER_LOT").ToString, FechaIni, dtDetalleAlm.Rows(j).Item("PROCEDENCIA").ToString, dtDetalleAlm.Rows(j).Item("PAIS_ORIGEN").ToString, dtDetalleAlm.Rows(j).Item("VALORACION").ToString, "")
                    Next
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub btnConsultar_Click(sender As Object, e As EventArgs) Handles btnConsultar.Click
        ...
        _Estado = dgvCabeceraAlmacen.Item(9, dgvCabeceraAlmacen.CurrentRow.Index).Value
        txt_Tipo_Ref.Text = Ref_documentid
        txt_Numero_Ref.Text = Ref_numerodocument
    ...
    Private Sub GenerarIngresoSalidaAlmacen(ByVal _Flag_Tipo As String)
        Try
            ...
            Dim alm_cabBE_ingreso As ClsEntidades.WAREHOUSE_TRANS
            ...
            For i = 0 To dgvDetalle.Rows.Count - 1
                ...
                With alm_detBE
                    ...
                    .NUMBER_ANALIS = dgvDetalle.Rows(i).Cells(4).Value
                    'almacenBL.Actualiza_valoracion_lote(.PART_ID, .NUMBER_LOT, dgvDetalle.Rows(i).Cells(13).Value)
                End With
                ls_det_alm.Add(alm_detBE)
            Next
            If almacenBL.GuardarInventarioAlmacen_Control_Calidad_PT(alm_cabBE, ls_det_alm) Then
                msj = "Documento Generado Correctamente," & vbCrLf & " Nro Documento: " & alm_cabBE.DOCUMENT_ID & " - " & alm_cabBE.NUMBER_DOCUMENT & vbCrLf & ""
                MsgBox(msj, MsgBoxStyle.Information)
                imprimirIngresoAlmacen(CboAlmacen.SelectedValue.ToString, "NS", alm_cabBE.NUMBER_DOCUMENT, alm_cabBE.USER_ID)
            Else
                Me.Cursor = Cursors.Default
                Exit Try
            End If
            'la parte de almacen de la NI
            alm_cabBE_ingreso = New ClsEntidades.WAREHOUSE_TRANS
            clsAlmacenCorrelativoBl = New ClsTransacciones.clsAlmacenCorrelativo
            ...
            Next
            If almacenBL.GuardarInventarioAlmacen_Control_Calidad_PT(alm_cabBE_ingreso, ls_det_alm_ingreso) Then
                msj = "Documento Generado Correctamente," & vbCrLf & " Nro Documento: " & alm_cabBE_ingreso.DOCUMENT_ID & " - " & alm_cabBE_ingreso.NUMBER_DOCUMENT & " "
                imprimirIngresoAlmacen(almacen_destino, "NI", alm_cabBE_ingreso.NUMBER_DOCUMENT, alm_cabBE_ingreso.USER_ID)
            Else
                Me.Cursor = Cursors.Default
                Exit Try
            End If
            ''''Cambiando el estado de liberacion
            almacenBL.Actualiza_Estado_Liberacion(Ref_idalamacen, Ref_documentid, Ref_numerodocument, _Flag_Tipo)
            almacenBL.Actualiza_Estado_Devolucion(txtTipoDocumentoRef.Text.Trim, txtNumeroReferencia.Text.Trim, _estado_control)
            '''''
            MsgBox(msj, MsgBoxStyle.Information)
            MsgBox(mensaje_final, MsgBoxStyle.Information)
            '' Envio de correo
            EnviarMail(CboAlmacen.SelectedValue.ToString, "NS", CORRELATIVO_NS)
            Me.Cursor = Cursors.Default
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    ...
    Private Sub imprimirIngresoAlmacen(ByVal idalamacen As String, ByVal documentid As String, ByVal numerodocument As String, ByVal USUARIOS As String)
        Try
            reporteBL = New ClsBuscar
            crystalBL = New LibReportes.ClsReporte
            Me.Cursor = Cursors.WaitCursor
            '--Ingreso Almacen
            dtImprimir = New DataTable("IngresoAlmacenPT")
            dtImprimir = reporteBL.EjecutarReporteGuiaAlmacen(idalamacen, documentid, numerodocument)
            If dtImprimir.Rows.Count() <> 0 Then
                crystalBL.Muestra_Reporte("rpt_Mov_Almacen_PT.rpt", "", "", "", "@almacen;" & idalamacen, "@tipo;" & documentid, "@numero;" & numerodocument, "@USER;" & USUARIOS)
            End If
            Me.Cursor = Cursors.Default
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub btnGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        For Each row As DataGridViewRow In dgvDetalle.Rows
            If String.IsNullOrEmpty(row.Cells(8).Value.ToString) Then
                Flag_indicador = True
                Exit For
            End If
        Next
        If Flag_indicador = True Then
            MsgBox("No se ingreso el numero de protocolo", MsgBoxStyle.Critical)
            Exit Sub
        End If
        For Each row As DataGridViewRow In dgvDetalle.Rows
            Dim AlmacenBl As New ClsOperaciones.WAREHOUSE_TRANS
            Dim dtTipoArt As New DataTable
            dtTipoArt = AlmacenBl.Mostrando_Clase_Articulo(row.Cells(1).Value)
            If dtTipoArt.Rows.Count() <> 0 Then
            ...
            End If
        Next
        If ListaAlm.Count() > 1 Then
            MsgBox("Los items pertenecen a distintos Almacenes.", MsgBoxStyle.Critical)
            Exit Sub
        Else
            '    If ListaAlm.Count() <> 0 Then
            If MsgBox("Desea continuar con la liberación de los productos?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Aviso") = MsgBoxResult.Yes Then
                mensaje_final = "Se aprobarón los productos."
                almacen_destino = ListaAlm(0)
                _estado_control = "APROBADO POR CONTROL"
    ...
    Private Sub EnviarMail(ByVal idalamacen As String, ByVal documentid As String, ByVal numerodocument As String)
        Try
            Dim envio_mail As New LibManufactura.FrmEnvioEmail
            envio_mail._almacen_id = idalamacen
            envio_mail._document_id = documentid
            envio_mail._number_document = numerodocument
            envio_mail._codigo_datos_email = "04"
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        Else
            If ListaAlm.Count() <> 0 Then
                If MsgBox("Desea continuar con el rechazo de los productos?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Aviso") = MsgBoxResult.Yes Then
                    mensaje_final = "Se rechazarón los productos."
                    almacen_destino = ListaAlm(0)
                    _estado_control = "RECHAZADO POR CONTROL"
