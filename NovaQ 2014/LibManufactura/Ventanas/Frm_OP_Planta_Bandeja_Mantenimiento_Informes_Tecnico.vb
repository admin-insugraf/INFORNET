...
Public Class Frm_OP_Planta_Bandeja_Mantenimiento_Informes_Tecnico
    ...
    Dim CAB_QUEJAS_RECLAMOS As ClsEntidades.QUEJAS_RECLAMOS
    ...
    Private Sub Frm_OP_Planta_Bandeja_Mantenimiento_Informes_Tecnico_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub DocumentosRequisiones()
        Try
            ...
            dtv = New DataView
            STRorden = ""
            dtDocumentos = clsPedidoBl.get_Quejas_Reclamos(dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value.ToString("dd/MM/yyyy"), _
            ...
            If dtDocumentos.Rows.Count() <> 0 Then
                ...
                dgvCabecera.Columns("TIPO").HeaderText = "TIPO"
                dgvCabecera.Columns("CLIENTE").HeaderText = "CLIENTE"
                dgvCabecera.Columns("PRODUCTO").HeaderText = "PRODUCTO"
                dgvCabecera.Columns("ESTADO").HeaderText = "ESTADO"

                dgvCabecera.Columns("NUMERO").Width = 100
                dgvCabecera.Columns("FECHA").Width = 100
                dgvCabecera.Columns("TIPO").Width = 200
                dgvCabecera.Columns("CLIENTE").Width = 250
                dgvCabecera.Columns("PRODUCTO").Width = 250
                dgvCabecera.Columns("ESTADO").Width = 100
                
                STRorden = dgvCabecera.Columns("NUMERO").Name & "+" & dgvCabecera.Columns("CLIENTE").Name & "+" & dgvCabecera.Columns("PRODUCTO").Name
    ...
    Private Sub btnGrabar_Click(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        Try
            If MsgBox("Esta seguro de registrar la Atención de la Queja y Reclamo?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Cuidado!") = MsgBoxResult.Yes Then
                Me.Cursor = Cursors.WaitCursor
                clsPedidoBl = New LibManufactura.ClsOperaciones.ORDERS
                CAB_QUEJAS_RECLAMOS = New ClsEntidades.QUEJAS_RECLAMOS
                ...
                With CAB_QUEJAS_RECLAMOS
                    ...
                    .FECHA_ACONTECIMIENTO = dtp_fecha_acontecimiento.Value.ToString("dd/MM/yyyy")
                    .REDACCION_HECHOS = txt_redaccion_hechos.Text
                    .SOLICITA_CLIENTE = txt_solicita_cliente.Text
                    .ACCIONES_INMEDIATAS = txt_acciones_inmediatas.Text
                    .ACCIONES_CORRECTIVAS = txt_acciones_correctivas.Text
                    .CONCLUSION = txt_conclusion.Text
                    .STATUS = cbo_estado.Text
                    .USER_PC = LibComunVar.ClsVarComun.USUARIO
                    .PC_ID = Environment.MachineName
                    .NOMBRE_RESPONSABLE = txt_nombre_responsable.Text
                    .CARGO_RESPONSABLE = txt_cargo_responsable.Text
                End With

                Dim _numero_queja_reclamo As Integer
                If clsPedidoBl.Guardar_Queja_Reclamo(CAB_QUEJAS_RECLAMOS, Modo_nuevo, _numero_queja_reclamo) Then
                    ...
                    frm_envio._producto = txt_producto.Text
    ...
    Private Function Validar_Registro_informes() As Boolean
        Try
            If cbo_tipo.SelectedIndex = -1 Then
                MsgBox("Debe elegir un Tipo.", MsgBoxStyle.Exclamation)
                cbo_tipo.Focus()
                Return False
            End If
            ...
            If txt_producto.Text = "" Then
                MsgBox("Debe elegir un producto.", MsgBoxStyle.Exclamation)
                txt_producto.Focus()
                Return False
            End If
            If txt_redaccion_hechos.Text = "" Then
                MsgBox("Debe registrar la redacción de los hechos.", MsgBoxStyle.Exclamation)
                txt_redaccion_hechos.Focus()
                Return False
            End If
    ...
    Private Sub MostrarModoConsultar()
        Try
            ...
            dtdetalleArticuloPrincipal = clsPedidoBl.get_Queja_Reclamo_Detalle(txtcodigo.Text)
            If dtdetalleArticuloPrincipal.Rows.Count() <> 0 Then
                dtp_fecha.Value = dtdetalleArticuloPrincipal.Rows(0).Item("FECHA")
                cbo_tipo.Text = dtdetalleArticuloPrincipal.Rows(0).Item("TIPO")
                ...
                txt_Cargo.Text = dtdetalleArticuloPrincipal.Rows(0).Item("CARGO")
                txt_correo.Text = dtdetalleArticuloPrincipal.Rows(0).Item("CORREO_ELECTRONICO")
                txt_telefono.Text = dtdetalleArticuloPrincipal.Rows(0).Item("TELEFONO")
                txt_producto.Text = dtdetalleArticuloPrincipal.Rows(0).Item("PRODUCTO")
                dtp_fecha_acontecimiento.Value = dtdetalleArticuloPrincipal.Rows(0).Item("FECHA_ACONTECIMIENTO")
                txt_redaccion_hechos.Text = dtdetalleArticuloPrincipal.Rows(0).Item("REDACCION_HECHOS")
                txt_solicita_cliente.Text = dtdetalleArticuloPrincipal.Rows(0).Item("SOLICITA_CLIENTE")
                txt_acciones_inmediatas.Text = dtdetalleArticuloPrincipal.Rows(0).Item("ACCIONES_INMEDIATAS")
                txt_acciones_correctivas.Text = dtdetalleArticuloPrincipal.Rows(0).Item("ACCIONES_CORRECTIVAS")
                txt_conclusion.Text = dtdetalleArticuloPrincipal.Rows(0).Item("CONCLUSION")
                cbo_tipo.Text = dtdetalleArticuloPrincipal.Rows(0).Item("STATUS")

                txt_nombre_responsable.Text = dtdetalleArticuloPrincipal.Rows(0).Item("NOMBRE_RESPONSABLE")
                txt_cargo_responsable.Text = dtdetalleArticuloPrincipal.Rows(0).Item("CARGO_RESPONSABLE")

                dtdetalleArchivos = clsPedidoBl.get_Detalle_Archivos_Quejas_Reclamos(txtcodigo.Text)
    ...
    Private Sub limpiar()
        ...
        txt_telefono.Text = ""
        txt_producto.Text = ""
        dtp_fecha_acontecimiento.Value = Date.Now
        txt_redaccion_hechos.Text = ""
        txt_solicita_cliente.Text = ""
        txt_ruta_archivo_dd.Text = ""
        txt_descripcion_archivo_dd.Text = ""
        txt_acciones_inmediatas.Text = ""
        txt_acciones_correctivas.Text = ""
        txt_conclusion.Text = ""
        TabControl1.SelectedIndex = 0
        cbo_estado.SelectedIndex = 0
        txt_nombre_responsable.Text = ""
        txt_cargo_responsable.Text = ""
        ...
        Else
            ...
            If dtv.Count() = 0 Then
                ...
                If dtv.Count() = 0 Then
                    ...
                    If dtv.Count() = 0 Then
                        STRorden = String.Empty
                        STRorden = dgvCabecera.Columns("PRODUCTO").Name
                        dtv.RowFilter = STRorden & " like '%" & txtFiltro.Text & "%'"
                        If dtv.Count() = 0 Then
                            STRorden = String.Empty
                            STRorden = dgvCabecera.Columns("NUMERO").Name & "+" & dgvCabecera.Columns("CLIENTE").Name & "+" & dgvCabecera.Columns("PRODUCTO").Name
    ...
    Private Sub dgvCabecera_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles dgvCabecera.CellFormatting
        'If dgvCabecera.Rows(e.RowIndex).Cells("ESTADO").Value = "Ejecución Total" Then
        ...
        'ElseIf dgvCabecera.Rows(e.RowIndex).Cells("ESTADO").Value = "Ejecución Parcial" Then
        '    dgvCabecera.Rows(e.RowIndex).DefaultCellStyle.ForeColor = Color.Blue
        '    dgvCabecera.Rows(e.RowIndex).DefaultCellStyle.SelectionForeColor = Color.Blue
        'Else
        ...
        'TipoTrade = (dgvCabecera.Rows(e.RowIndex).Cells("NIVEL_IMPORTANCIA").Value)

        'If TipoTrade = "Normal" Then
        '    e.CellStyle.ForeColor = Color.Black
        '    e.CellStyle.SelectionForeColor = Color.White
        'End If

        'If TipoTrade = "Urgente" Then
        '    e.CellStyle.ForeColor = Color.Red
        '    e.CellStyle.SelectionForeColor = Color.Orange
        'End If

        'Try
        '    If Me.dgvCabecera.Columns(e.ColumnIndex).Name = "NIVEL_IMPORTANCIA" Then
        '        If e.Value IsNot Nothing Then
        '            Dim valor As String = e.Value
        '            If (valor = "Urgente") Then
    ...
    Private Sub Ayuda_Solicitante()
        Try
            ...
            If frm.Data_Matriz.Rows.Count > 0 Then
                ...
                txt_correo.Text = frm.Data_Matriz.Rows(0).Item("EMAIL").ToString
                txt_telefono.Text = frm.Data_Matriz.Rows(0).Item("TELEFONO").ToString
            End If
            frm.Close()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub btn_agregar_archivo_dd_Click(sender As Object, e As EventArgs) Handles btn_agregar_archivo_dd.Click
        ...
        Try
            ...
            txtcodigo.Text = "0"
            dt_item_archivo = clsPedidoBl.get_item_archivo_Quejas_Reclamos(txtcodigo.Text)
            Dim item As Integer
            ...
            If clsPedidoBl.Grabar_Archivo_Queja_Reclamo(txtcodigo.Text, datos_archivo) Then
                dtdetalleArchivos = clsPedidoBl.get_Detalle_Archivos_Quejas_Reclamos(txtcodigo.Text)
                dgv_lista_archivos_dd.DataSource = dtdetalleArchivos
    ...
    Private Sub dgv_lista_archivos_dd_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_lista_archivos_dd.CellClick
        ...
        If e.ColumnIndex = 0 Then 'descargar archivo
            Try
                ...
                dt_datos_proyecto_descarga = clsPedidoBl.get_datos_archivo_descargar_Queja_Reclamo(txtcodigo.Text, Codigo)
        ...
        If e.ColumnIndex = 1 Then 'eliminar archivo
            Try
                ...
                If MessageBox.Show("Se va a eliminar el archivo" & vbCrLf & "¿Desea continuar?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    Me.Cursor = Cursors.WaitCursor
                    If clsPedidoBl.Eliminar_Archivo_Queja_Reclamo(txtcodigo.Text, item) Then
                        dtdetalleArchivos = clsPedidoBl.get_Detalle_Archivos_Quejas_Reclamos(txtcodigo.Text)
                        dgv_lista_archivos_dd.DataSource = dtdetalleArchivos
                    End If
    ...
    Private Sub txt_Cargo_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_Cargo.KeyDown
        If e.KeyCode = Keys.Enter Then
            txt_correo.Focus()
        End If
    End Sub
    ...
    Private Sub txt_producto_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_producto.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            Else
                dtp_fecha_acontecimiento.Focus()
            End If
        End If
    End Sub
    ...
    Private Sub Mostrando_Producto_Insumos()
        Try
            ...
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
                txt_producto.Text = frm.Data_Matriz.Rows(0).Item(1).ToString
                txt_producto.Select()
            End If
            frm.Close()
        Catch ex As Exception
