...
Public Class Frm_Generar_Orden_Desarrollo
    ...
    Private Sub Frm_Generar_Orden_Desarrollo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub MostrandoDatosGeneralesCabeceraAlmacen()
        Try
            ...
            dtData = clsBusquedaBl.get_DatosGenerales_Ordenes_Desarrollo(dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value.ToString("dd/MM/yyyy"), _
            ...
            If dtData.Rows.Count() <> 0 Then
                ...
                dgvCabeceraAlmacen.Columns("COD_PRODUCTO").Visible = False 'HeaderText = "Cod. Producto"
    ...
    Private Sub imprimirOp(ByVal _Codigo As Integer, ByVal _Almacen As String)
        Try
            ...
            Me.Cursor = Cursors.WaitCursor
            dtImprimir_OP = reporteBL.Get_Imprimir_Orden_Desarrollo(_Codigo)
            dtDatosCompany = reporteBL.Obtener_Datos_Empresa()

            If dtImprimir_OP.Rows.Count() <> 0 Then
                crystalBL.Muestra_Reporte("rpt_Orden_Desarrollo_Insugraf_ii.rpt", dtImprimir_OP, "", "", _
                                          "@ID_CAB;" & _Codigo, _
    ...
    Private Sub EliminarDocumentosAlmacen(ByVal opcion_ As Integer)
        Try
            ...
            Me.Cursor = Cursors.WaitCursor
            If almacenBL.Eliminar_Orden_Desarrollo(opcion_) Then
    ...
    Private Sub btnGrabar_Click_1(sender As Object, e As EventArgs) Handles btnGrabar.Click
        ...
        '    Exit Sub
        'Next
        If MessageBox.Show("Se va a registrar la Orden de Desarrollo" & vbCrLf & "¿Desea continuar?", "Eliminar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
        ...
        Try
            ...
            For i As Integer = 0 To dtdetalleArticuloPrincipal_productos.Rows.Count() - 1
                ...
                With alm_cabBE
                    ...
                    .DESCRIPCION = dtdetalleArticuloPrincipal_productos.Rows(i).Item("DESCRIPCION").ToString
                    .UNIDAD = dtdetalleArticuloPrincipal_productos.Rows(i).Item("UNIDAD").ToString
            ...
            Me.Cursor = Cursors.WaitCursor
            '--PARTE DE ALMACEN
            If almacenBL.Guardar_Orden_Desarrollo(alm_cab, ls_det, ls_det_alm, ls_det_alm_procesos, Modo_consultar) Then
                Dim msj As String = String.Empty
                msj = String.Empty
                If Modo_consultar = False Then
                    msj = "Generación de Orden de Desarrollo correctamente." '& vbCrLf & " Nro Documento: " & "NS" & " - " & alm_cabBE_almacen.NUMBER_DOCUMENT
                Else
                    msj = "Modificación de Orden de Desarrollo correctamente." '& vbCrLf & " Nro Documento: " & "NS" & " - " & alm_cabBE_almacen.NUMBER_DOCUMENT
                End If
                MsgBox(msj, MsgBoxStyle.Information)
                Me.Cursor = Cursors.Default
            End If
    ...
    Private Function MostrarModoEdicion(ByVal _codigo As Integer) As Boolean
        Try
            ...
            dtCabeceraAlm = clsAlmacenCabBl.get_Cabecera_Orden_Desarrollo(_codigo)
            If dtCabeceraAlm.Rows.Count() <> 0 Then
                ...
                dtdetalleArticuloPrincipal_productos = clsAlmacenDetBl.get_Detalle_Orden_Desarrollo_Productos(_codigo)
                ...
                If dtdetalleArticuloPrincipal_productos.Rows.Count() <> 0 Then
                    ...
                    dgvDetalle.Columns(2).HeaderText = "Cliente"
                    dgvDetalle.Columns(3).HeaderText = "Fecha de Entrega"
                    dgvDetalle.Columns(4).Visible = False 'HeaderText = "Precio"
                    dgvDetalle.Columns(5).HeaderText = "Vendedor"
                    dgvDetalle.Columns(6).HeaderText = "Destinatario"
                    dgvDetalle.Columns(7).HeaderText = "Urgente"
                    dgvDetalle.Columns(8).Visible = False 'HeaderText = "Saldo"
                    dgvDetalle.Columns(9).Visible = False 'HeaderText = "Codigo"
                    dgvDetalle.Columns(10).HeaderText = "Producto"
                    dgvDetalle.Columns(11).HeaderText = "Unidad"
                    dgvDetalle.Columns(12).HeaderText = "Cantidad"
                ...
                dtdetalleArticuloPrincipal = clsAlmacenDetBl.get_Detalle_Orden_Desarrollo(_codigo)

                '---Mostrando datos del Detalle
                dtdetalleArticuloPrincipal_Procesos = clsAlmacenDetBl.get_Detalle_Orden_Desarrollo_Procesos(_codigo)
    ...
    Public Sub GenerarColummnaDataTable_Productos()
        Try
            ...
            dgvDetalle.Columns(7).HeaderText = "Urgente"
            dgvDetalle.Columns(8).Visible = False 'HeaderText = "Saldo"
            dgvDetalle.Columns(9).HeaderText = "Codigo"
            dgvDetalle.Columns(10).HeaderText = "Producto"
            dgvDetalle.Columns(11).HeaderText = "Unidad"
            dgvDetalle.Columns(12).HeaderText = "Cantidad"
            dgvDetalle.Columns(13).Visible = False 'HeaderText = "Saldo"
            dgvDetalle.Columns(14).Visible = False 'HeaderText = "Saldo"
            dgvDetalle.Columns(15).HeaderText = "Stock"
    ...
    Private Sub txt_cod_principio_activo_MouseDoubleClick(sender As Object, e As MouseEventArgs)
    ...
    Private Sub ConfigurarGrila_Productos()
        If dtdetalleArticuloPrincipal_productos.Rows.Count() <> 0 Then
            ...
            dgvDetalle.Columns(12).DefaultCellStyle.Format = "N2"
            dgvDetalle.Columns(12).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight

            dgvDetalle.Columns(15).DefaultCellStyle.Format = "N2"
            dgvDetalle.Columns(15).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        End If
    ...
    Private Sub txt_factor_KeyDown(sender As Object, e As KeyEventArgs)
        If e.KeyCode = Keys.Enter Then
    ...
    Private Sub btnAprobar_Click(sender As Object, e As EventArgs) Handles btnAprobar.Click
        If MessageBox.Show("Se va a APROBAR la Orden de Desarrollo" & vbCrLf & "¿Desea continuar?", "Aprobar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
    ...
    Private Function Generar_Descarga_Insumos() As Boolean
        Try
            ...
            If almacenBL.Guardar_Descarga_Orden_Desarrollo(alm_cabBE, alm_cabBE_almacen, ls_det_alm_almacen) Then
                Dim msj As String = String.Empty
                msj = String.Empty
                msj = "Orden de Desarrollo aprobado correctamente." '& vbCrLf & " Nro Documento: " & "NS" & " - " & alm_cabBE_almacen.NUMBER_DOCUMENT
                MsgBox(msj, MsgBoxStyle.Information)
                Me.Cursor = Cursors.Default
            End If
            Return True
    ...
    Function ValidacionesGeneralesArticulo() As Boolean
        ...
        If txt_des_prod_producir.Text.Trim = "" Then
            MsgBox("Debe ingresar una descripcion del Producto para Producir.", MsgBoxStyle.Exclamation, "Sistemas")
            txt_des_prod_producir.Focus()
            Return False
            Exit Function
        End If

        If txt_unidad_prod_producir.Text.Trim = "" Then
            MsgBox("Debe elegir una unidad de medida.", MsgBoxStyle.Exclamation, "Sistemas")
            txt_unidad_prod_producir.Focus()
            Return False
            Exit Function
        End If
    ...
    Private Sub Ayuda_OrdenesCompra()
        Try
            ...
            sql = "COMPRA_SP_S_GENERACION_ORDENES_DESARROLLO"
            frm.CadenaConsulta = sql
            frm.Titulo = "LISTA DE MUESTRAS PARA DESARROLLO"
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
            ...
            dt_req_op = clsAlmacenCabBl.get_Detalle_Produccion_OP_Productos_Muestras_Desarrollo(ID_COMPRA)
    ...
    Private Sub txt_unidad_prod_producir_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles txt_unidad_prod_producir.MouseDoubleClick
        Ayuda_Unidad_Medida()
    End Sub

    Private Sub txt_unidad_prod_producir_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_unidad_prod_producir.KeyDown
        If e.KeyCode = Keys.F1 Then
            Ayuda_Unidad_Medida()
        End If
        If e.KeyCode = Keys.Enter Then
            If txt_unidad_prod_producir.Text = "" Then
                Ayuda_Unidad_Medida()
            Else
                txt_cantidad_prod_producir.Focus()
            End If
    ...
    Private Sub Ayuda_Unidad_Medida()
        Try
            ...
            frm.ShowDialog()
            If frm.Data_Matriz.Rows.Count > 0 Then
                txt_unidad_prod_producir.Text = frm.Data_Matriz.Rows(0).Item(0).ToString
            End If
            frm.Close()
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub txt_des_prod_producir_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_des_prod_producir.KeyDown
        If e.KeyCode = Keys.Enter Then
            txt_unidad_prod_producir.Focus()
        End If
    End Sub
