...
Public Class ClsTransacciones
    ...
    Public Class FORMULACION
        ...
        Public Sub Get_NumeroCorrelativoFormulacion()
            Try
                Dim dtdetalles As New DataTable
                dtdetalles = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "CTR_SP_S_CORRELATIVO").Tables(0)
    ...
    Public Class ORDER_PRODUCTION
        ...
        Public Sub Get_NumeroCorrelativoOrdenProduccion(ByVal MES As Integer, ANHO As Integer)
            Try
                Dim dtdetalles As New DataTable
                dtdetalles = SqlHelper.ExecuteDataset(cn, "CTR_SP_S_CORRELATIVO_ORDEN_P", MES, ANHO).Tables(0)
                NumeroCorrelativo = dtdetalles.Rows(0).Item("LAST_NUMBER").ToString
            Catch ex As Exception
                Throw
            End Try
        ...
        Public Sub Get_NumeroCorrelativo_Lote()
            Try
                Dim dtdetalles As New DataTable
                dtdetalles = SqlHelper.ExecuteDataset(cn, "CTR_SP_S_CORRELATIVO_LOTE_OP").Tables(0)
                NumeroCorrelativo = dtdetalles.Rows(0).Item("LAST_NUMBER").ToString
            Catch ex As Exception
                Throw
            End Try
