...
Public Class Frm_Parte_Entrega
    ...
    Private Sub Frm_Parte_Entrega_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If LibComunVar.ClsVarComun.ACT_PE = "SI" Then
            btn_act_costos.Visible = True
        Else
            btn_act_costos.Visible = False
        End If
    ...
    Private Sub btnAgregar_Click(sender As Object, e As EventArgs) Handles btnAgregar.Click
        ...
        If CboAlmacen.SelectedIndex = -1 Then
            MsgBox("Debe elegir un almacen. Verifique!!!", MsgBoxStyle.Exclamation)
            Exit Sub
        End If
    ...
    Private Sub ModificandoArticulos()
        Try
            'GbCabecera.Enabled = False
            Dim frmArticulo As New FrmAsignarArticulo_Parte_Entrega
            ...
            For Each item As DataRow In dtdetalleArticuloPrincipal.Select("ITEM=" & Codigo)
                ...
                row("AREA") = item("AREA")
                row("F_VENCIMIENTO") = item("F_VENCIMIENTO")
                row("F_FABRICACION") = item("F_FABRICACION")
            ...
            If frmArticulo.fl_close = False Then
                If frmArticulo.dtdetalleArticulo.Rows.Count() <> 0 Then
                    For Each item As DataRow In dtdetalleArticuloPrincipal.Select("ITEM=" & Codigo)
                        ...
                        item("AREA") = frmArticulo.dtdetalleArticulo.Rows(0).Item("AREA")
                        item("F_VENCIMIENTO") = frmArticulo.dtdetalleArticulo.Rows(0).Item("F_VENCIMIENTO")
                        item("F_FABRICACION") = frmArticulo.dtdetalleArticulo.Rows(0).Item("F_FABRICACION")
    ...
    Private Sub MostrandoDatosGeneralesCabeceraAlmacen()
        Try
            ...
            dtData = clsBusquedaBl.get_DatosGeneralesAlmacen_E_S_Parte_Entrega(dtpfechaInicial.Value.ToString("dd/MM/yyyy"), dtpfechafinal.Value.ToString("dd/MM/yyyy"), _
                                                                 cboopcionesBusqueda.SelectedIndex, TipoMov, _documentId, LibComunVar.ClsVarComun.AccesoAlmacenes, _
                                                                 cboAlmacenBusqueda.SelectedValue.ToString)
            dgvCabeceraAlmacen.DataSource = Nothing
            If dtData.Rows.Count() <> 0 Then
                dtv = dtData.DefaultView
                dgvCabeceraAlmacen.DataSource = dtv
                dgvCabeceraAlmacen.Columns("ID_CAB").HeaderText = "N°"
                dgvCabeceraAlmacen.Columns("FECHA").HeaderText = "FECHA"
                dgvCabeceraAlmacen.Columns("ESTADO").HeaderText = "ESTADO"
                dgvCabeceraAlmacen.Columns("DOC_REF").HeaderText = "TIPO REFERENCIA"
                dgvCabeceraAlmacen.Columns("NUMERO_DOC_REF").HeaderText = "N° REFERENCIA"
                dgvCabeceraAlmacen.Columns("ALM_REF").Visible = False
                dgvCabeceraAlmacen.Columns("FECHA_REGISTRO").HeaderText = "FECHA/HORA"
    ...
    Private Sub Nuevo()
        Try
            ...
            cboTipoMovimiento.SelectedValue = "IP"
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    End Sub
    ...
    Private Function ValidacionGeneral(ByRef idalamacen As String, ByRef documentid As String, ByRef numerodocument As String) As Boolean
        ...
        Try
            'Asignacion
            idalamacen = dgvCabeceraAlmacen.CurrentRow.Cells("ALM_REF").Value
            documentid = dgvCabeceraAlmacen.CurrentRow.Cells("DOC_REF").Value
            numerodocument = dgvCabeceraAlmacen.CurrentRow.Cells("NUMERO_DOC_REF").Value
            'validacion
    ...
    Private Sub MostrarModoEdicion(ByVal idAlmacen As String, ByVal documentId As String, ByVal NumeroDocument As String)
        Try
            ...
            If dtCabeceraAlm.Rows.Count() <> 0 Then
                ...
                dtdetalleArticuloPrincipal = clsAlmacenDetBl.get_DetalleAlmacen_Parte_Entrega(idAlmacen, documentId, NumeroDocument, dgvCabeceraAlmacen.CurrentRow.Cells("ID_CAB").Value)
                ...
                '        dgvDetalle.Rows.Add(dtDetalleAlm.Rows(j).Item("ITEM").ToString, dtDetalleAlm.Rows(j).Item("NUMBER_ANALIS").ToString, _
                '                            dtDetalleAlm.Rows(j).Item("PART_ID").ToString, dtDetalleAlm.Rows(j).Item("PART_DESCRIPTION").ToString, _
                '          dtDetalleAlm.Rows(j).Item("UNIT_PART").ToString, dtDetalleAlm.Rows(j).Item("QTY").ToString, _
                '         dtDetalleAlm.Rows(j).Item("QT_BULTOS").ToString, IIf(dtDetalleAlm.Rows(j).Item("OPT_ENTREGA").ToString = "0", "Parcial", "Total"), _
                '         dtDetalleAlm.Rows(j).Item("NUM_PROTOCOLOS").ToString, dtDetalleAlm.Rows(j).Item("NUMBER_LOT").ToString, dtDetalleAlm.Rows(j).Item("AMOUNT").ToString)
                '    Next
                'End If
                ...
                dtCabeceraAlm = clsAlmacenCabBl.get_CabeceraAlmacen_Parte_Entrega_validacion(idAlmacen, documentId, NumeroDocument, dgvCabeceraAlmacen.CurrentRow.Cells("ID_CAB").Value)
                If dtCabeceraAlm.Rows.Count() <> 0 Then
                    btn_act_costos.Enabled = False
                Else
                    btn_act_costos.Enabled = True
    ...
    Private Function GenerarIngresoSalidaAlmacen() As Boolean
        Try
            ...
            If Modo_actualizar = True Then
                ...
                For i As Integer = 0 To dtdetalleArticuloPrincipal.Rows.Count() - 1
                    ...
                    With alm_detBE
                        ...
                        .F_VCTO = dtdetalleArticuloPrincipal.Rows(i).Item("F_VENCIMIENTO").ToString
                        .F_FABRICACION = dtdetalleArticuloPrincipal.Rows(i).Item("F_FABRICACION").ToString
                        .WAREHOUSE_REF = ""
                        .ACCOUNT = _ACCOUNT
            ...
            If Modo_actualizar = True Then
                If almacenBL.ActualizarInventarioAlmacen_Parte_Entrega(alm_cabBE, ls_det_alm, txtloteFabricar.Text) Then
                ...
                End If
            Else
                If almacenBL.GuardarInventarioAlmacen_Parte_Entrega(alm_cabBE, ls_det_alm, txtloteFabricar.Text) Then
                    'msj = "Nuevo Documento Generado Correctamente" & vbCrLf & "Nro Documento: " & documentId & " - " & alm_cabBE.NUMBER_DOCUMENT
                    msj = "Parte de Entrega Generado Correctamente"
    ...
    Private Sub AnularDocumentosAlmacen()
        Try
            ...
            Dim almacenBL As New ClsOperaciones.WAREHOUSE_TRANS
            Me.Cursor = Cursors.WaitCursor
            almacenBL.AnularDocumentosAlmacen_Parte_Entrega(Ref_idalamacen, Ref_documentid, Ref_numerodocument, Usuario, Num_Oc, dgvCabeceraAlmacen.CurrentRow.Cells("ID_CAB").Value)
            Dim msj As String = String.Empty
            msj = "Documento anulado Correctamente." '& Ref_numerodocument
            MsgBox(msj, MsgBoxStyle.Information)
            MostrandoDatosGeneralesCabeceraAlmacen()
            Me.Cursor = Cursors.Default
        Catch ex As Exception
    ...
    Private Sub imprimirAlmacen(ByVal _idalamacen As String, ByVal _documentid As String, ByVal _numerodocument As String)
        Try
            ...
            Me.Cursor = Cursors.WaitCursor
            dtImprimir = reporteBL.EjecutarReporteGuiaAlmacen_Parte_Entrega(_idalamacen, _documentid, _numerodocument, dgvCabeceraAlmacen.CurrentRow.Cells("ID_CAB").Value)
            If dtImprimir.Rows.Count() <> 0 Then
                crystalBL.Muestra_Reporte("rpt_Mov_Almacen_Parte_Entrega_I.rpt", dtImprimir, "", "", "@almacen;" & _idalamacen, "@tipo;" & _documentid, "@numero;" & _numerodocument)
            End If
            Me.Cursor = Cursors.Default
        Catch ex As Exception
            Me.Cursor = Cursors.Default
    ...
    Private Sub btnConsultar_Click_1(sender As Object, e As EventArgs) Handles btnConsultar.Click
        ...
        gbCabceraDocumento.Enabled = True
        gbLeyenda.Visible = False
        dgvdetalleAlmacen.Visible = True 'False
        dgvDetalle.Visible = False 'True
    ...
    Private Sub Tool_Anular_Click(sender As Object, e As EventArgs) Handles Tool_Anular.Click
        ...
        ref_fecha_doc = dgvCabeceraAlmacen.Item(1, dgvCabeceraAlmacen.CurrentRow.Index).Value
        ...
        clsAlmacenCabBl = New LibCobranzas.ClsOperaciones.WAREHOUSE_TRANS
        dtCabeceraAlm = New DataTable
        dtCabeceraAlm = clsAlmacenCabBl.get_CabeceraAlmacen_Parte_Entrega_validacion("", "", "", dgvCabeceraAlmacen.CurrentRow.Cells("ID_CAB").Value)
        If dtCabeceraAlm.Rows.Count() <> 0 Then
            MsgBox("No se puede anular el Parte de Entrega" & Chr(13) & "El Parte de Entrega tiene traslados desde el almacen de Producción.", MsgBoxStyle.Critical)
            Exit Sub
        End If
    ...
    Private Sub MostrandoTipoTransacciones()
        Try
            ...
            ElseIf MostrarMovimientoBl.IS_VALUED = "V" Then
                chkValorizado.Visible = False 'True
    ...
    Private Sub Cargar_Almacen()
        ...
        dtAlmacen = almacenBL.get_Almacenes_Ayuda_Parte_Entrega(LibComunVar.ClsVarComun.AccesoAlmacenes)
    ...
    Public Sub GenerarColummnaDataTable()
        Try
            ...
            dtdetalleArticuloPrincipal.Columns.Add("F_VENCIMIENTO", Type.GetType("System.String"))
            dtdetalleArticuloPrincipal.Columns.Add("F_FABRICACION", Type.GetType("System.String"))
            ...
            dgvdetalleAlmacen.Columns(2).HeaderText = "Descripción"
            ...
            dgvdetalleAlmacen.Columns("OBSERVACIONES").HeaderText = "Observaciones"
            dgvdetalleAlmacen.Columns("AREA").HeaderText = "Area"
            dgvdetalleAlmacen.Columns("F_VENCIMIENTO").HeaderText = "Fecha Vcto."
            dgvdetalleAlmacen.Columns("F_FABRICACION").HeaderText = "Fecha Fabricación"
            ...
            'If LibComunVar.ClsVarComun.RucEmpresa = "20556850454" Then
            '    dgvdetalleAlmacen.Columns("LOTE").Visible = True
            '    dgvdetalleAlmacen.Columns("F_VENCIMIENTO").Visible = True
            'Else
            '    dgvdetalleAlmacen.Columns("LOTE").Visible = False
            '    dgvdetalleAlmacen.Columns("F_VENCIMIENTO").Visible = False
            'End If

            dgvdetalleAlmacen.Columns("NRO_PROTOCOLO").Visible = False
            dgvdetalleAlmacen.Columns("NRO_BULTOS").Visible = False
            dgvdetalleAlmacen.Columns("MONTO").Visible = False
            dgvdetalleAlmacen.Columns("OBSERVACIONES").Visible = True
            dgvdetalleAlmacen.Columns("AREA").Visible = True

            dgvdetalleAlmacen.Columns(12).HeaderText = "N° O.P."
            dgvdetalleAlmacen.Columns(12).DisplayIndex = 1
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
    End Sub
    ...
    Private Sub AgregandoArticulos()
        Try
            ...
            If frmArticulo.dtdetalleArticulo.Rows.Count() <> 0 Then
                ...
                row("OBSERVACIONES") = frmArticulo.dtdetalleArticulo.Rows(0).Item("OBSERVACIONES")
                row("AREA") = frmArticulo.dtdetalleArticulo.Rows(0).Item("AREA")
                row("F_VENCIMIENTO") = frmArticulo.dtdetalleArticulo.Rows(0).Item("F_VENCIMIENTO")
    ...
    Private Sub AgregandoArticulos_Quimicos()
        Try
            Dim frmArticulo As New FrmAsignarArticulo_Parte_Entrega_Quimicos
            frmArticulo.EsValorizado = IIf(chkValorizado.Checked, True, False)
            frmArticulo.Flag_ModoEdicion = False
            frmArticulo.AlmacenOrigen = CboAlmacen.SelectedValue.ToString
            frmArticulo.TipoMov = TipoMov
            ...
            frmArticulo.ShowDialog()
            If frmArticulo.dtdetalleArticulo.Rows.Count() <> 0 Then
                For i As Integer = 0 To frmArticulo.dtdetalleArticulo.Rows.Count() - 1
                    Dim row As DataRow = dtdetalleArticuloPrincipal.NewRow
                    row("ITEM") = dtdetalleArticuloPrincipal.Rows.Count() + 1
                    row("CODIGO") = frmArticulo.dtdetalleArticulo.Rows(i).Item("CODIGO")
                    row("DESCRIPCION") = frmArticulo.dtdetalleArticulo.Rows(i).Item("DESCRIPCION")
                    row("UNIDAD") = frmArticulo.dtdetalleArticulo.Rows(i).Item("UNIDAD")
                    row("LOTE") = frmArticulo.dtdetalleArticulo.Rows(i).Item("LOTE")
                    row("CANTIDAD") = frmArticulo.dtdetalleArticulo.Rows(i).Item("CANTIDAD")
                    row("NRO_PROTOCOLO") = frmArticulo.dtdetalleArticulo.Rows(i).Item("NRO_PROTOCOLO")
                    row("NRO_BULTOS") = frmArticulo.dtdetalleArticulo.Rows(i).Item("NRO_BULTOS")
                    row("MONTO") = frmArticulo.dtdetalleArticulo.Rows(0).Item("MONTO")
                    row("OPT_ENTREGA") = frmArticulo.dtdetalleArticulo.Rows(i).Item("OPT_ENTREGA")
                    row("PROCEDENCIA") = frmArticulo.dtdetalleArticulo.Rows(i).Item("PROCEDENCIA")
                    row("PAIS_ORIGEN") = frmArticulo.dtdetalleArticulo.Rows(i).Item("PAIS_ORIGEN")
                    row("NUMERO_ANALISIS") = frmArticulo.dtdetalleArticulo.Rows(i).Item("NUMERO_ANALISIS")
                    row("OBSERVACIONES") = frmArticulo.dtdetalleArticulo.Rows(i).Item("OBSERVACIONES")
                    row("F_VENCIMIENTO") = frmArticulo.dtdetalleArticulo.Rows(i).Item("F_VENCIMIENTO")
                    row("F_FABRICACION") = frmArticulo.dtdetalleArticulo.Rows(i).Item("F_FABRICACION")
                    dtdetalleArticuloPrincipal.Rows.Add(row)
                    dtdetalleArticuloPrincipal.AcceptChanges()
                    If frmArticulo.dtDetallesBulto.Rows.Count() > 0 Then
                        If frmArticulo.Flag_ModoEdicion = False Then
