...
Public Class FrmReporteProducto_Cuarentena
    ...
    Public Sub MostrarReportes(ByVal Familia As String, ByVal art_inicial As String, ByVal art_final As String)
        Try
            reporteBL = New ClsReporteGenerales
            crystalBL = New LibReportes.ClsReporte
            Me.Cursor = Cursors.WaitCursor
            If NombreReporte = "MP" Then
                dtReporte = New DataTable("Producto_MP")
                dtReporte = reporteBL.Imprimir_Producto_Cuarentena_MP(Familia, art_inicial, art_final)
                If dtReporte.Rows.Count() <> 0 Then
                    crystalBL.Muestra_Reporte("rpt_producto_cuarentena_MP.rpt", "", "", "", "@FAMILIA;" & Familia, "@PROD_INI;" & art_inicial, "@PROD_FIN;" & art_final)
                Else
                    MsgBox("No hay informacion dispnible para Mostrar", MsgBoxStyle.Information, "Sistemas")
                End If
            ElseIf NombreReporte = "PT" Then
                dtReporte = New DataTable("Producto_PT")
                dtReporte = reporteBL.Imprimir_Producto_Cuarentena_PT(Familia, art_inicial, art_final)
                If dtReporte.Rows.Count() <> 0 Then
                    crystalBL.Muestra_Reporte("rpt_producto_cuarentena_PT.rpt", dtReporte, "", "", "@FAMILIA;" & Familia, "@PROD_INI;" & art_inicial, "@PROD_FIN;" & art_final)
                Else
                    MsgBox("No hay informacion dispnible para Mostrar", MsgBoxStyle.Information, "Sistemas")
                End If
            End If
    ...
    Private Sub btnMostrar_Click(sender As Object, e As EventArgs) Handles btnMostrar.Click
        If chk_articulos.Checked = False Then
            ...
            End If
        End If
        MostrarReportes(IIf(chkfamilia.Checked = True, "", cboFamilia.SelectedValue.ToString), txtinicioArticulo.Text, txtFinArticulo.Text)
    End Sub
