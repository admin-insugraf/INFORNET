...
CREATE procedure [dbo].[PUR_LIST_TB_BANK]
--declare
...
as
	...
	if len(@codigo)>0
		set @criterio=@criterio + N' and P.BANK_ID=@codigo'

        if @opcion='0'   --Buscar si existe el código de banco
    	   set @STRquery='Select BANK_ID  from TB_BANK P'

        if @opcion='1'   -- Editar datos del banco
    	   set @STRquery='Select *  from TB_BANK P'

        if @opcion='2'   -- Mostrar datos para el GRID
	set @STRquery='Select BANK_ID,NAME   from TB_BANK'
