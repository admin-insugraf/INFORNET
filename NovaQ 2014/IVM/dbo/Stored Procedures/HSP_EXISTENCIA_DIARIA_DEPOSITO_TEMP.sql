CREATE procedure [dbo].[HSP_EXISTENCIA_DIARIA_DEPOSITO_TEMP]  
 --declare  
 @EMP varchar(3),  
 @COD_DMAY varchar(10),  
...
--*/as  
 ...
 if exists(select * from tempdb.dbo.sysobjects WHERE NAME='STOCK_DIARIO_TEMP'+@PC)  
  EXEC('drop table [tempdb].dbo.STOCK_DIARIO_TEMP'+@PC+'')  
  
  exec('create table [tempdb].dbo.STOCK_DIARIO_TEMP'+@PC+'  
    (COD_OPER varchar(10),  
    COD_MAYOR int,  
 ...
 WHILE @inicio<=@fin  
  BEGIN  
   ...
   exec('insert into [tempdb].dbo.STOCK_DIARIO_TEMP'+@PC+' (COD_OPER,COD_MAYOR,COD_OSINERG,FECHA,COD_PRODUCTO,DES_PRODUCTO,COD_ALMACEN,CANT_INGRESO,CANT_SALIDA,  
