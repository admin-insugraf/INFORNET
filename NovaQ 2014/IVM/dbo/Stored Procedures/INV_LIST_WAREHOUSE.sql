--******* CREADO 09/05/2007 *********
--****** PROC - MOSTRAR DATOS DE ALMACENES*****
CREATE procedure [dbo].[INV_LIST_WAREHOUSE]
--declare
...
as
	...
	if len(@codigo)>0
        ...
        if @opcion='0'   --Buscar si existe el código del almacen
    	   set @STRquery='Select id  from WAREHOUSE P'

        if @opcion='1'   -- Editar datos del almacen
    	   set @STRquery='Select *  from WAREHOUSE P'

        if @opcion='2'   -- Mostrar datos para el GRID
    	   set @STRquery='Select * FROM WAREHOUSE '
