--******* CREADO 07/06/2007 *********
--****** PROC - MOSTRAR TIPO DE ORDEN DE IMPORTACION*****
CREATE procedure [dbo].[PUR_LIST_TYPE_IMPORT]
--declare
...
as
	...
	if len(@codigo)>0
        ...
        if @opcion='0'   --Buscar si existe el código tipo de orden de importacion
    	   set @STRquery='Select ID from TYPE_IMPORT P'

        if @opcion='1'   -- Editar datos de tipo de orden de importacion
    	   set @STRquery='Select *  from TYPE_IMPORT P'

        if @opcion='2'   -- Mostrar datos para el GRID
	set @STRquery='Select ID,DESCRIPTION   from TYPE_IMPORT'
