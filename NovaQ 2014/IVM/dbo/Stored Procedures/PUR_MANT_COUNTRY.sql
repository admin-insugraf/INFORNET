...
CREATE   procedure [dbo].[PUR_MANT_COUNTRY]
--declare   
...
declare @Errores int
  ...
  if @OPCION='1' --Insertar  
    Set @STRquery='INSERT INTO COUNTRY (COUNTRY_ID, NAME )    
                  VALUES (@codigo,@nombre)'  
  if @OPCION='2'  --Modificar  
     Set @STRquery='UPDATE COUNTRY SET NAME=@nombre  WHERE COUNTRY_ID=@codigo'  

  if @OPCION='3'  --Eliminar
     Set @STRquery='DELETE FROM COUNTRY WHERE COUNTRY_ID=@codigo'  
...
if @errores<>0 
   BEGIN  
         ROLLBACK TRAN Mant_Country
    END   
else  
   BEGIN  
         COMMIT TRAN Mant_Country
   END