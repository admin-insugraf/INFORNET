...
Public Class FrmAsignarArticulo
    ...
    Public ValidacionLotes As List(Of FrmFacturacion.StructuraPart)
    ...
    Public Flag_Factura_Exportacion As Boolean = False
    ...
    Public _valida_stock As Boolean = False
    ...
    Private Sub ArticulosPrecioDescuentos()
        Try
            ...
            txt_ultimo_precio.Text = Format(clsDescuento.DCTO4, "##,##0.00")
            txt_lista_precio.Text = Format(clsDescuento.EFFECTIVE_PRICE, "##,##0.00")
    ...
    Private Sub MostrandoArticulos()
        Try
            ...
            If Flag_Factura_directa = False And Flag_Factura_Exportacion = False Then
    ...
    Private Sub AgregarArticuloManual(ByVal _codigoArticulo As String)
        Try
            ...
            dtDocumentoGuias = clsArticuloBl.get_VerificacionArticuloCodigoFactura(AlmacenOrigen, _codigoArticulo, IIf(Flag_Factura_directa = True Or Flag_Factura_Exportacion = True, True, False), TipoDocumento, Flag_Factura_Pedido, _ARTS_INICIALES)
            ...
            If dtDocumentoGuias.Rows.Count() <> 0 Then
                ...
                If Flag_Modo_Edicion = False Then
                    If Flag_Factura_Exportacion = True Then
                        txtigvpor.Text = Math.Round(CDbl(0), 2)
                    Else
                ...
                If Flag_FacturaExistencias = False Then
                    If dtDocumentoGuias.Rows(0).Item("IS_LOT").ToString = "S" Then
                        ...
                        MostrandoLotes(txtcodigoArticulo.Text)
                        If Flag_Factura_Pedido Then
                            If cboLotes.Items.Count > 0 Then cboLotes.SelectedIndex = 0
                        Else
                            cboLotes.Select()
                        End If
            ...
            Else
                ...
                txtStockLote.Text = String.Empty
                cboLotes.DataSource = Nothing
                txt_ultimo_precio.Text = Format(0, "##,##0.00")
                txt_lista_precio.Text = Format(0, "##,##0.00")
                txtcodigoArticulo.Focus()
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Function ValidacionesGenerales() As Boolean
        ...
        Try
            ...
            If txtigvpor.Text = "" Then
                MsgBox("El I.G.V. no puede estar en blanco.", MsgBoxStyle.Exclamation)
                estado = False
                txtprecioUnitario.Focus()
                Exit Try
            End If
            If Val(txtigvpor.Text) < 0 Then
                MsgBox("El I.G.V. debe ser mayor a Cero", MsgBoxStyle.Exclamation)
                estado = False
                txtigvpor.Focus()
                Exit Try
            End If
            ...
            'End If
            If _valida_stock = True Then
                FLAG_VALIDA_STOCK = "S"
            ElseIf _valida_stock = False Then
                FLAG_VALIDA_STOCK = "N"
            End If
                        ...
                        If Flag_FacturaGuiaVenta = False Then
                            If _TipoControlInventario = "LOTE" Then
                                If CDbl(IIf(txtStockLote.Text = "", 0, txtStockLote.Text)) < CDbl(txtcantidad.Text) Then
                                    MsgBox("La cantidad debe ser menor o igual que el  Stock por Lote.", MsgBoxStyle.Exclamation)
                                    estado = False
                                    txtcantidad.Select()
                                    Exit Try
                                End If
                            ...
                            If Flag_Factura_Pedido = True Then
                                If CDbl(IIf(txtsaldo.Text = "", 0, txtsaldo.Text)) < CDbl(txtcantidad.Text) Then
                                    MsgBox("La cantidad debe ser menor o igual que el  saldo.", MsgBoxStyle.Exclamation)
                                    estado = False
                                    txtcantidad.Select()
                                    Exit Try
                                End If
    ...
    Private Sub btnAceptar_Click(sender As Object, e As EventArgs) Handles btnAceptar.Click
        ...
        If Flag_Modo_Edicion = False Then
            ...
            row("IMP_DESC_02") = 0
            row("PRECIO_IGV") = 0
            row("IGV_ART") = 0 'CDbl(txtigvpor.Text)
            row("IGV_ART_US") = 0 'CDbl(txtigvpor.Text)
    ...
    Private Sub txtcodigoArticulo_Leave(sender As Object, e As EventArgs) Handles txtcodigoArticulo.Leave
        ...
        Dim dtTableFecha As New DataTable
        Dim LibData As New LibConexion.ClsData
        cadena = "SELECT B.FLAG_REQUI FROM dbo.PART A INNER JOIN dbo.PART_TYPE B ON A.TYPE_PART=B.ID WHERE A.ID='" & txtcodigoArticulo.Text & "' "
        Dim FLAG_EDICION As String = String.Empty
        dtTableFecha = LibData.Run_Query_DataTable(cadena)
        If dtTableFecha.Rows.Count() <> 0 Then
            FLAG_EDICION = dtTableFecha.Rows(0).Item(0).ToString()
        End If

        'If txtcodigoArticulo.Text = "N0008" Or txtcodigoArticulo.Text = "N0006" Or Strings.Left(Trim(txtcodigoArticulo.Text), 2) = "ND" Or Strings.Left(Trim(txtcodigoArticulo.Text), 2) = "NC" Or FLAG_EDICION = "1" Then
        '    txtdescripcionArticulo.ReadOnly = False
        '    txtdescripcionArticulo.BackColor = Drawing.Color.White
        'Else
        '    txtdescripcionArticulo.ReadOnly = True
