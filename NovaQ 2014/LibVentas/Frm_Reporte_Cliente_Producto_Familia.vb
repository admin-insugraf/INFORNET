...
Imports LibReportes
Public Class Frm_Reporte_Cliente_Producto_Familia
    Dim reporteBL As ClsBuscar
    Dim crystalBL As ClsReporte
    Dim dtImprimir As DataTable

    Private Sub Frm_Reporte_Cliente_Producto_Familia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            If chk_productos.Checked = False Then
                If txtProductoInicial.Text = String.Empty Then
                    MsgBox("Debe seleccionar una Familia.", MsgBoxStyle.Exclamation)
    ...
    Private Sub ImprimirReporte()
        Try
            ...
            dtImprimir = reporteBL.EjecutarReporteVentasClienteProducto_Familia(IIf(checkVendedor.Checked = True, "", txtClienteInicial.Text), dtpfechaInicio.Value.ToString("dd/MM/yyyy"), dtpfechaFin.Value.ToString("dd/MM/yyyy"), IIf(chk_productos.Checked = True, "", txtProductoInicial.Text))
            If dtImprimir.Rows.Count() <> 0 Then
                If rdb_detallado.Checked = True Then
                    crystalBL.Muestra_Reporte("rpt_Ventas_Cliente_x_producto_familia.rpt", dtImprimir, "", "", "@CLIENTE;" & txtClienteInicial.Text, "@FECHA_INI;" & dtpfechaInicio.Value.ToString("dd/MM/yyyy"), "@FECHA_FIN;" & dtpfechaFin.Value.ToString("dd/MM/yyyy"), "@PRODUCTO;" & txtProductoInicial.Text, "FECHA_INI;" & dtpfechaInicio.Value.ToString("dd/MM/yyyy"), "FECHA_FIN;" & dtpfechaFin.Value.ToString("dd/MM/yyyy"))
                ElseIf rdb_resumido.Checked = True Then
                    crystalBL.Muestra_Reporte("rpt_Ventas_Cliente_x_producto_resumido_familia.rpt", dtImprimir, "", "", "@CLIENTE;" & txtClienteInicial.Text, "@FECHA_INI;" & dtpfechaInicio.Value.ToString("dd/MM/yyyy"), "@FECHA_FIN;" & dtpfechaFin.Value.ToString("dd/MM/yyyy"), "@PRODUCTO;" & txtProductoInicial.Text, "FECHA_INI;" & dtpfechaInicio.Value.ToString("dd/MM/yyyy"), "FECHA_FIN;" & dtpfechaFin.Value.ToString("dd/MM/yyyy"))
                End If
            Else
