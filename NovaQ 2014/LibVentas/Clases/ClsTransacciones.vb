...
Public Class ClsTransacciones
    ...
    Public Class COMPANY_TYPE
        ...
        Public Function Mostrar_Detalles_Pedido(ByVal NUMERO As String) As DataTable
            Try
                Return SqlHelper.ExecuteDataset(cn, "PED_SP_S_ORDER_LINE_HAB_VISTA_PRELIMINAR", NUMERO).Tables(0)
    ...
    Public Class DEVOLUTION
        ...
        Private _NumeroLineas As Double
        ...
        Public Property NumeroLineas As Double
            Get
        ...
        Public Sub Get_NumeroCorrelativoDevolucion()
            Try
                Dim dtdetalles As New DataTable
                dtdetalles = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "DEV_SP_S_NUMERACION").Tables(0)
                If dtdetalles.Rows.Count() Then
                    NumeroCorrelativo = dtdetalles.Rows(0).Item("LAST_NUMBER").ToString
                    NumeroSerie = dtdetalles.Rows(0).Item("NUMBER_SERIE").ToString
                    NumeroLineas = dtdetalles.Rows(0).Item("NUMBER_LINE").ToString
                End If
    ...
    Public Class clsAlmacen
        ...
        Public Sub Get_NumeroFactura(ByVal _num_serie As String)
            Try
                Dim dtGuiaRemision As New DataTable
                'dtGuiaRemision = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "FACT_SP_S_RECEIVABLE_DOCUMENT_CODIGO", _num_serie).Tables(0)
                dtGuiaRemision = SqlHelper.ExecuteDataset(cn, "FACT_SP_S_RECEIVABLE_DOCUMENT_CODIGO", _num_serie).Tables(0)
                For i As Integer = 0 To dtGuiaRemision.Rows.Count() - 1
                    TYPE_DOC = dtGuiaRemision.Rows(i).Item("TYPE_DOC").ToString
                    NUMBER_SERIE = dtGuiaRemision.Rows(i).Item("NUMBER_SERIE").ToString
                    LAST_NUMBER = dtGuiaRemision.Rows(i).Item("LAST_NUMBER").ToString
            ...
            End Try
        End Sub
        '--------GENERACION DE NOTAS DE CREDITO
        Public Sub Get_NumeroFacturaNotaCredito()
            Try
                Dim dtNotaCredito As New DataTable
                dtNotaCredito = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "FACT_SP_S_R_DOCUMENT_CODIGO_NOTAC").Tables(0)
                For i As Integer = 0 To dtNotaCredito.Rows.Count() - 1
                    TYPE_DOC = dtNotaCredito.Rows(i).Item("TYPE_DOC").ToString
                    NUMBER_SERIE = dtNotaCredito.Rows(i).Item("NUMBER_SERIE").ToString
                    LAST_NUMBER = dtNotaCredito.Rows(i).Item("LAST_NUMBER").ToString
                    NUMBER_LINE = dtNotaCredito.Rows(i).Item("NUMBER_LINE").ToString
                Next
            Catch ex As Exception
                Throw
            End Try
        End Sub
        '--------GENERACION DE NOTAS DE DEBITO
        Public Sub Get_NumeroFacturaNotaDebito()
            Try
                Dim dtNotaDebito As New DataTable
                dtNotaDebito = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "FACT_SP_S_R_DOCUMENT_CODIGO_NOTAD").Tables(0)
                For i As Integer = 0 To dtNotaDebito.Rows.Count() - 1
                    TYPE_DOC = dtNotaDebito.Rows(i).Item("TYPE_DOC").ToString
                    NUMBER_SERIE = dtNotaDebito.Rows(i).Item("NUMBER_SERIE").ToString
                    LAST_NUMBER = dtNotaDebito.Rows(i).Item("LAST_NUMBER").ToString
                    NUMBER_LINE = dtNotaDebito.Rows(i).Item("NUMBER_LINE").ToString
                Next
            Catch ex As Exception
                MsgBox(ex.Message)
                Throw
        ...
        Public Sub Get_NumeroFacturaComprobantePercepcion()
            Try
                Dim dtComprobantePer As New DataTable
                dtComprobantePer = SqlHelper.ExecuteDataset(cn, CommandType.StoredProcedure, "FACT_SP_S_R_DOCUMENT_CODIGO_CP").Tables(0)
                For i As Integer = 0 To dtComprobantePer.Rows.Count() - 1
                    TYPE_DOC = dtComprobantePer.Rows(i).Item("TYPE_DOC").ToString
                    NUMBER_SERIE = dtComprobantePer.Rows(i).Item("NUMBER_SERIE").ToString
                    LAST_NUMBER = dtComprobantePer.Rows(i).Item("LAST_NUMBER").ToString
                    NUMBER_LINE = dtComprobantePer.Rows(i).Item("NUMBER_LINE").ToString
                Next
            Catch ex As Exception
                MsgBox(ex.Message)
                Throw
    ...
    Public Class clsAlmacenCorrelativo
        ...
        Public Sub Get_NumeroCorrelativoAlmacen(ByVal codigoAlmacen As String)
            Try
                Dim dtguiaAnalisis As New DataTable
                dtguiaAnalisis = SqlHelper.ExecuteDataset(cn, "ALM_SP_S_WAREHOUSE_CORRELATIVO", codigoAlmacen).Tables(0)
                For i As Integer = 0 To dtguiaAnalisis.Rows.Count() - 1
                    NumeroCorrelativo = dtguiaAnalisis.Rows(i).Item("LAST_NUM_IN").ToString
                Next
            Catch ex As Exception
                MsgBox(ex.Message)
                Throw
